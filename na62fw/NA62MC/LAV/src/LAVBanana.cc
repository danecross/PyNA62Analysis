...
LAVBanana::LAVBanana(G4int bananaId, LAVVPbGlBlock* pbglBlock)
{
  ...
  // Coordinates of center of vessel in slab coordinates system (to be computed later)
  G4double xC = 0.; G4double yC = 0.;

  // Create vector with slab vertices (will be empty for station 12)
  ...
  // Not used for banana type 3 (Station 12)
  if (fBananaId==0 || fBananaId==1 || fBananaId==2) {
    ...
    G4ExtrudedSolid* alSlabSolid =
      new G4ExtrudedSolid("bananaAlSlab",vtx,halfSlabThick,G4TwoVector(),1.,G4TwoVector(),1.);
    ...
    xC = 0.;
    yC = -(sqrt(fBananaOuterRadius*fBananaOuterRadius-x5*x5)-y5);
    ...
    blockRAtCenter = sideDist-fPbGlBlock->GetBlockZofBackFace();
    */
  ...
  for(G4int iBlock=0;iBlock<fNBlocks;iBlock++) {
    ...
    // Create rotation matrix for this block
    ...
    // Create translation vector for this block
    G4double blockRAtCenter = 0.;
    if (fBananaId == 3) {
      blockRAtCenter = fBananaInnerRadius+fPbGlBlock->GetBlockZofFrontFace();
    } else {
      G4double xs = 0.; G4double ys = 0.;
      if (iBlock == 0) {
	xs = 0.5*(vtx[1].x()+vtx[2].x());
	ys = 0.5*(vtx[1].y()+vtx[2].y());
      } else if (iBlock == 1) {
	xs = 0.5*vtx[1].x();
	ys = 0.5*vtx[1].y();
      } else if (iBlock == 2) {
	...
	xs = 0.5*(vtx[10].x()+vtx[9].x());
	ys = 0.5*(vtx[10].y()+vtx[9].y());
      }
      blockRAtCenter = sqrt((xs-xC)*(xs-xC)+(ys-yC)*(ys-yC))-fPbGlBlock->GetBlockZofBackFace();
    }
    // Correct computed radius with (small) tolerance, if any
    G4double blockR = blockRAtCenter+geoParams->GetBananaExtraRadiusTolerance(bananaId,iBlock);
    G4ThreeVector translationBlock = G4ThreeVector(blockR*sin(phiBlock),blockR*cos(phiBlock),0.);
