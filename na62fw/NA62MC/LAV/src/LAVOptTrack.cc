...
G4bool LAVOptTrack::ProcessOpticalPhoton(G4Step *aStep){
...
//      LAVGeometryParameters *geoParams = LAVGeometryParameters::GetInstance();
      fTimeMatrix.Load(geoParams->GetLAVTimeMatrix());
      fEfficiencyMatrix.Load(geoParams->GetLAVEfficiencyMatrix());
   ...
   while (i != v->end())
      if ((*i)->GetDefinition() == fOpticalPhoton &&
          (*i)->GetVolume()->GetName()=="LAV_PbGlBlock"){
         toret = true;
   ...
   if(aStep->GetTrack()->GetDefinition() == fOpticalPhoton &&
      aStep->GetTrack()->GetVolume()->GetName()=="LAV_PbGlBlock"){
      toret = true;
...
void LAVOptTrack::Process(const G4ThreeVector &PosGb,
