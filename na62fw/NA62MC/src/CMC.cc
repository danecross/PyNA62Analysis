...
void kch2munuXscal(TLorentzVector, double, double);
void kch2munuXvec(TLorentzVector, double, double);
...
void CMC::InitPhotos() {
  if (!fPhotosInitialised) {
    phoini();
    na48phoin();
    G4cout << "[CMC] Photos initialised" << G4endl;
...
void CMC::Kch2munuXscal()  {
  kch2munuXscal(Get4Momentum(fBeamIndex-1), fExoticParticleMass[0], fExoticParticleLifetime[0]);
}

///////////////////////
// K+ --> mu+ nu X'vec

void CMC::Kch2munuXvec()  {
  kch2munuXvec(Get4Momentum(fBeamIndex-1), fExoticParticleMass[0], fExoticParticleLifetime[0]);
}
/////////////////////////
// K+ --> pi0 e+ nu (Ke3)
