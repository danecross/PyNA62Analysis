...
MUV3MaterialParameters* MUV3MaterialParameters::fInstance = 0;

MUV3MaterialParameters::MUV3MaterialParameters() {
  // Prepare here every Material Property Table
  DefineMaterials();
  SetMaterialProperties();
}

MUV3MaterialParameters* MUV3MaterialParameters::GetInstance() {
  if (!fInstance) fInstance = new MUV3MaterialParameters();
  return fInstance;
}

void MUV3MaterialParameters::DefineMaterials() {
  ...
  nistMgr->FindOrBuildMaterial("G4_GLASS_LEAD");
  ...
  /*
  G4Material* StainlessSteel = new G4Material("MUV3_StainlessSteel",7.88*g/cm3,4); //Prepend subdetector name
  ...
  */
  // Define 96% Pb + 4% Sn for MUV3 absorber
  // Density calculated to be: 0.04 * 7.365 + 0.96 * 11.34 = 11.181 g/cm3
  /*
  G4Material* PbSnAlloy = new G4Material("MUV3_PbSnAlloy",11.181*g/cm3,2);
...
void MUV3MaterialParameters::SetMaterialProperties() {
  // Use here the material property tables prepared
}

TObjArray MUV3MaterialParameters::GetHashTable() {
  return 0;
}
