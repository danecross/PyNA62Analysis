...
IRCScintillatorLayer::IRCScintillatorLayer(G4Material * Material, G4LogicalVolume * MotherVolume, 
					   G4Transform3D Transform3D,
					   G4double InnerRadius, G4double OuterRadius, G4int iCopy) : 
...
IRCScintillatorLayer::~IRCScintillatorLayer(){}

void IRCScintillatorLayer::ReadGeometryParameters()
{
  ...
  fZLength = GeoPars->GetScintillatorLayerZLength();
  fXdisplacement = GeoPars->GetIRCInnerHoleDisplacementX();
}

void IRCScintillatorLayer::CreateGeometry()
{
  ...
  fSolidVolume = new G4SubtractionSolid("IRCScintillatorLayer",plate,hole,Rotation,Displacement);

  fLogicalVolume= new G4LogicalVolume(fSolidVolume,                      // solid
				      fMaterial,                         // material
				      "IRCScintillatorLayer",            // name
				      0,                                 // field manager 
  ...
  fPhysicalVolume = new G4PVPlacement(fTransform3D,
				      ...
				      "IRCScintillatorLayer",            // its name
				      fMotherVolume,                     // its mother  volume
				      false,                             // no boolean operations
				      fiCopy);                           // copy number

  G4SDManager* SDman = G4SDManager::GetSDMpointer();
  G4String IRCSDname = "/IRC";
  G4VSensitiveDetector * IrcSD = SDman->FindSensitiveDetector(IRCSDname);
  fLogicalVolume->SetSensitiveDetector(IrcSD);
}

void IRCScintillatorLayer::SetProperties()
{
