...
CHANTIMaterialParameters* CHANTIMaterialParameters::fInstance = 0;

CHANTIMaterialParameters::CHANTIMaterialParameters()
{
...
CHANTIMaterialParameters* CHANTIMaterialParameters::GetInstance()
{
  if ( fInstance == 0 ) { fInstance = new CHANTIMaterialParameters(); }
  return fInstance;
}

void CHANTIMaterialParameters::DefineMaterials()
{
  ...
  nistMgr->FindOrBuildMaterial("G4_DIOXIDE_SILICON");
  ...
  nistMgr->FindOrBuildMaterial("G4_POLYACRYLONITRILE");

  // Materials to be specific for this subdetector
  // Example: Stainless Steel (StainlessSteel)
  G4Material* StainlessSteel = new G4Material("CHANTI_StainlessSteel",7.88*g/cm3,4); //Prepend subdetector name
...
void CHANTIMaterialParameters::SetMaterialProperties()
{
  // Use here the material property tables prepared
}

TObjArray CHANTIMaterialParameters::GetHashTable()
{
  TObjArray CHANTIMaterialParameters;
  ...
  CHANTIMaterialParameters.Add(new DetectorParameter("fMaterialPropertiesNEntries",Value.Data(),
					       "Material Properties N Entries", ParameterData));
  ParameterData.Clear();


  return CHANTIMaterialParameters;
}

void CHANTIMaterialParameters::Print(){
