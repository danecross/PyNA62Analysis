...
LKrEMShowers::LKrEMShowers(G4String modelName, G4Region* envelope) :
  ...
  rc = loadOrSaveDb(db, "shower.db", false);
  if (rc != SQLITE_OK) {
    issuewithfile = true;
    G4cout << "[LKrEMShowers] Attention! there is a problem with the DB file" << G4endl;  
    return;
  }
