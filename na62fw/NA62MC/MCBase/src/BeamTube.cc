...
void BeamTube::CreateGeometry()
{
  ...
  fSolidVolume= new G4Tubs("BeamTubeEnvelope",
			   0,
			   fOuterRadius,
			   HalfZLength,
			   startPhiAngle,
  ...
  fPhysicalVolume = new G4PVPlacement(0,                         // no rotation
				      ...
				      "BeamTubeEnvelope",                // its name
				      fMotherVolume,              // its mother  volume
				      false,                     // no boolean operations
				      0);                        // copy number

  G4Tubs* SolidBeamTube = new G4Tubs("BeamTube",
				     fInnerRadius,
				     fOuterRadius, 					     
				     HalfZLength,
				     startPhiAngle,
  ...
  fLogicalBeamTube = new G4LogicalVolume(SolidBeamTube,        // solid
					 fMaterial,             // material
					 "BeamTube",           // name
					 0,                    // field manager
					 0,                    // sensitive detector
  ...
  fPhysicalBeamTube = new G4PVPlacement(0,               // no rotation
					G4ThreeVector(), // at (0,0,0)
					fLogicalBeamTube,      // its logical volume
					"BeamTube",         // its name
  ...
  //------------------------------ 
  new G4LogicalSkinSurface("Tube Surface",fLogicalBeamTube,fOpticalSurface);   
...
void BeamTube::DefineOpticalSurface()
{
  ...
  fOpticalSurface = new G4OpticalSurface("BeamTubeOptical");
...
{
  fVisAtt= new G4VisAttributes(G4Colour(0.0,0.0,0.0));
  fVisAtt->SetVisibility(false);
  fLogicalVolume ->SetVisAttributes(fVisAtt); 
  G4VisAttributes* BeamTubeVisAtt= new G4VisAttributes(G4Colour(0.0,1.0,1.0));
  fLogicalBeamTube->SetVisAttributes(BeamTubeVisAtt);
}
