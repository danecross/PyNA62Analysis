...
BeamPipe::BeamPipe(G4int SegmentIndex, G4Material * EnvMaterial, NA62VGeometryParameters* GeoPars, G4LogicalVolume * MotherVolume, G4bool EnableFins) :
NA62VComponent(EnvMaterial,MotherVolume), NA62VNamed("Pipe"), fSegmentIndex(SegmentIndex), fEnableFins(EnableFins), fInnerRadius(0.), fOuterRadius(0.), fBeamTube(nullptr), fBeamTubeFins(nullptr), fGeoPars(GeoPars), fOpticalSurface(nullptr) {
...
void BeamPipe::CreateGeometry()
{
   ...
   //------------------------------
      ...
      if(fEnableFins) fBeamTubeFins = new BeamTubeFins(fSegmentIndex,G4Material::GetMaterial("G4_Al"),fGeoPars,fLogicalVolume);
...
// BeamPipe Reflective Surface
  ...
  if(fEnableFins) new G4LogicalBorderSurface("Gas/BeamTubeFins Surface",
