...
GigaTrackerReconstruction::GigaTrackerReconstruction(TFile* HistoFile, TString ConfigFileName) :
  ...
  fParameters(nullptr), fHTrueMomentum(nullptr), fHTrueThetaX(nullptr),
  fHTrueThetaY(nullptr), fHTrueMomentumX(nullptr), fHTrueMomentumY(nullptr),
  fHTrueMomentumZ(nullptr)
{
  // INSTANCIATE PRIVATE OBJECTS
  fRecoEvent = new TRecoGigaTrackerEvent();
  fSlimRecoEvent = new TSlimRecoGigaTrackerEvent();
  mHistErrSet = false;
  ...
  for(int i(0);i<3;i++){
    // Reset station time offsets
    for(int k(0);k<18000;k++)    fT0[i][k] = 0;       //< pixel tzero
    for(int j(0);j<10;j++)       fTW[i][j] = nullptr; //< chip time walk

    // reset histograms
    fHHitMap[i]                = nullptr;
    fHAbsTimeProfile[i]        = nullptr;
    fHAbsTimeProfileZoom[i]    = nullptr;
    fHHitMapOOB[i]             = nullptr;
    fHAbsTimeProfileOOB[i]     = nullptr;
    fHAbsTimeProfileZoomOOB[i] = nullptr;
    fHCoarseHitMap[i]          = nullptr;

    for (int j(0); j<11; ++j){
      fHToT[i][j]                = nullptr;
      fHTime[i][j]               = nullptr;
      fHTimeProfile[i][j]        = nullptr;
      fHRawTime[i][j]            = nullptr;
      fHRawTimeProfile[i][j]     = nullptr;
      fHNHitsPerTrigger[i][j]    = nullptr;
      fHToTOOB[i][j]             = nullptr;
      fHTimeOOB[i][j]            = nullptr;
      fHNHitsPerTriggerOOB[i][j] = nullptr;

      fNHitsPerTriggerAll[i][j]    = -999.;
      fNHitsPerTrigger[i][j]       = -999.;
      fNHitsPerTriggerOOB[i][j]    = -999.;
      fNHitsPerTriggerOOBAll[i][j] = -999.;
    }
  }
...
void GigaTrackerReconstruction::Init(NA62VReconstruction* MainReco){
  ...
  fMC = 0;
  if (!static_cast<NA62Reconstruction*>(fMainReco)->GetIsRawData()) fMC = 1;
  if (!fMC){
    LoadTW();
    LoadT0();
  }
...
void GigaTrackerReconstruction::ParseConfFile(TString ConfFileName){
  ...
  while(Line.ReadLine(confFile)){
    ...
    if (Line.BeginsWith("EnableRawTimeCorrections")) {
...
TRecoVEvent * GigaTrackerReconstruction::ProcessEvent(TDetectorVEvent* tEvent, Event* tGenEvent){
  ...
  if (!fParameters){
    ...
    fParameters = GigaTrackerParameterTools::GetInstance(RunID,fMC);
