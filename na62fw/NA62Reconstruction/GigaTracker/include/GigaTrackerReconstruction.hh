...
class GigaTrackerReconstruction : public NA62VReconstruction {
  ...
  private:
    ...
    template<typename Order>
      void Clusterize(std::vector<int>, std::vector<std::vector<int>>&, double minDist, Order order, int minCont);
    ...
    struct Cluster {
      ...
      void add(int i) {
        ...
        { std::cout<<"Cluster error: trying to add hit from different stations"<<std::endl; return;}
