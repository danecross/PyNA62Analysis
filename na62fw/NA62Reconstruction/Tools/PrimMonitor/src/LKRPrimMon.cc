...
LKRPrimMon::LKRPrimMon(TRootBrowser* mainwin, TString Name) : VPrimMon(mainwin, Name) {
  NBurst = 0 ;

  // set hist style
  fHPrimTime = new TH1F("fHPrimTimeLKR","LKR primitives rate                                        ",800, 0., 8000.);
  ...
  fHPrimTimeZoom1 = new TH1F("fHPrimTimeZoomLKR","LKR primitives rate                         ",80000, 0., 8000.);
  ...
  fHPrimDelta = new TH1F("fHPrimDeltaLKR","dt of primitives (ns) ;Time Difference (ns);Counts",200000, -100000, 100000.);
  fHPrimDelta->GetXaxis()->SetTitle("ns");
  fHPrimDelta->GetYaxis()->SetTitle("");
  fHPrimDelta->SetLineColor(kRed);

  fHPrimDeltaTS = new TH1F("fHPrimDeltaTSLKR","dt of primitives (FT units) ;Time Difference (x100 ps) ;Counts",2000, -1000, 1000.);
  fHPrimDeltaTS->GetXaxis()->SetTitle("FT units");
  fHPrimDeltaTS->GetYaxis()->SetTitle("");
  fHPrimDeltaTS->SetLineColor(kOrange);

  fHPrimID = new TH1F("fHPrimIDLKR","LKR primitive types",16, -0.5, 15.5);
  ...
  fHTimeBits = new TH1F("fHTimeBitsLKR","Probability of the 40 tstamp bits (timestamp & finetime) to be high; TimeStamp & FineTime; Fraction", 40, -39.5,0.5) ;
  fHTimeBits->Sumw2() ;

  fHFineTime = new TH1F("fHFineTimeLKR","Fine Time; FT; Counts", 256, -0.5,255.5) ;

  fHTimeStamp16   = new TH1F("fHTimeStamp16LKR","Time Stamp below 4th bit;Time Stamp mod 16 ; Count", 16, -0.5,15.5) ;
  fHTimeStamp128  = new TH1F("fHTimeStamp128LKR","Time Stamp below 7th bit;Time Stamp mod 128 ; Count", 128, -0.5,127.5) ;
  fHTimeStamp1024 = new TH1F("fHTimeStamp1024LKR","Time Stamp below 10th bit;Time Stamp mod 1024 ; Count", 256, -0.5,1023.5) ;
  fHTimeStamp8192 = new TH1F("fHTimeStamp8192LKR","Time Stamp below 13th bit;Time Stamp mod 8192 ; Count", 512, -0.5,8191.5) ;

  fHFineTimeStamp128   = new TH1F("fHFineTimeStamp128LKR",
				 "Fine Time Stamp below 7th bit; Fine; Count", 128, -0.5,127.5) ;

  fHPrimFFT= new TH1D("fHPrimFFTLKR","FFT - LKR; Hz; ",65536,0.,10000.);
  fHPrimFFT->SetLineColor(kBlue);

  fHPrimTimeFFT = new TH1D("fHPrimTimeFFTLKR","LKR time profile for FFT; ns",65536,0.,6.5536E+9);
  ...
  TCanvas * cLKRPrimTS = AddCanvasTab("Rate");
  cLKRPrimTS->Divide(1,3);
  cLKRPrimTS->cd(1)->SetPad(.005, .500, .995, .995);
  cLKRPrimTS->cd(1);
  fHPrimTime->Draw();
  fPText->Draw();
  cLKRPrimTS->cd(2)->SetPad(.500, .005, .995, .495);
  cLKRPrimTS->cd(2);
  cLKRPrimTS->cd(2)->SetLogy(kTRUE);
  fHPrimDeltaTS->Draw();
  cLKRPrimTS->cd(3)->SetPad(.005, .005, .495, .495);
  cLKRPrimTS->cd(3);
  cLKRPrimTS->cd(3)->SetLogy(kTRUE);
  fHPrimDelta->Draw();

  TCanvas * cLKRZoom = AddCanvasTab("Burst structure");
  cLKRZoom->Divide(1,3);
  cLKRZoom->cd(1)->SetPad(.005, .500, .500, .995 );
  cLKRZoom->cd(1);
  fHPrimTimeZoom1->Draw();
  cLKRZoom->cd(2)->SetPad(.500, .500, .995, .995);
  cLKRZoom->cd(2);
  fHPrimFFT->Draw();
  cLKRZoom->cd(3)->SetPad(.005, .005, .995, .495);
  cLKRZoom->cd(3);
  fHPrimTimeZoom2->GetXaxis()->SetRangeUser(3000.,4000.);
  fHPrimTimeZoom2->Draw();

  TCanvas * cLKRPrimID = AddCanvasTab("Primitive IDs");
  cLKRPrimID->cd(1)->SetLogx(kTRUE);
  fHPrimID->Draw("hbar1");
  fHPrimID->SetFillColor(kGreen);
  fHPrimID->SetBarWidth(0.8);

  /*TCanvas * cLKRBits = */AddCanvasTab("Time Bits");
  fHTimeBits->GetYaxis()->SetRangeUser(0.4,0.6);
  fHTimeBits->Draw();

  TCanvas * cLKRTimeStamps = AddCanvasTab("Time Stamps");
  cLKRTimeStamps->Divide(2,2) ;
  cLKRTimeStamps->cd(1) ;
  fHTimeStamp16->Draw();
  cLKRTimeStamps->cd(2) ;
  fHTimeStamp128->Draw();
  ...
  /*TCanvas * cLKRFineTime = */AddCanvasTab("Fine Time");
  fHFineTime->Draw() ;

  /*TCanvas * cLKRFineTimeStamps = */AddCanvasTab("MultiPad Fine Time Stamps");
...
void LKRPrimMon::Update(TTree* tree, TString TimeRunBurst,  TString time) {
  ...
  cout << "LKR entries " << nEntries << endl;
  ...
  for (int iEnt = 0; iEnt < nEntries; iEnt++) {
    ...
    if(iEnt%100000==0){
      cout << "LKR: " << (100.*iEnt)/nEntries << "% of processing complete \r";
    }
  ...
  fHPrimTime->SetTitle(Form("LKR primitives rate - %s", TimeRunBurst.Data()));
  fHPrimTimeZoom1->SetTitle(Form("LKR - %s", TimeRunBurst.Data()));
...
void LKRPrimMon::Rescale(TH1F* h){
