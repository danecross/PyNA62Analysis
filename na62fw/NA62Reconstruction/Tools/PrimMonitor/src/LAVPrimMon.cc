...
LAVPrimMon::LAVPrimMon(TRootBrowser* mainwin, TString Name) : VPrimMon(mainwin, Name) {
  NBurst = 0 ;

  // set hist style
\
  fHPrimTime = new TH1F("fHPrimTimeLAV","LAV primitives rate                                        ",800, 0., 8000.);
  ...
  fHPrimTimeZoom1 = new TH1F("fHPrimTimeZoomLAV","LAV primitives rate                         ",80000, 0., 8000.);
  ...
  fHPrimDelta = new TH1F("fHPrimDeltaLAV","dt of primitives (ns) ;Time Difference (ns);Counts",200000, -100000, 100000.);
  fHPrimDelta->GetXaxis()->SetTitle("ns");
  fHPrimDelta->GetYaxis()->SetTitle("");
  fHPrimDelta->SetLineColor(kRed);

  fHPrimDeltaTS = new TH1F("fHPrimDeltaTSLAV","dt of primitives (FT units) ;Time Difference (x100 ps) ;Counts",2000, -1000, 1000.);
  fHPrimDeltaTS->GetXaxis()->SetTitle("FT units");
  fHPrimDeltaTS->GetYaxis()->SetTitle("");
  fHPrimDeltaTS->SetLineColor(kOrange);

  fHPrimID = new TH1F("fHPrimIDLAV","LAV primitive types",16, -0.5, 15.5);
  ...
  fHTimeBits = new TH1F("fHTimeBitsLAV","Probability of the 40 tstamp bits (timestamp & finetime) to be high; TimeStamp & FineTime; Fraction", 40, -39.5,0.5) ;
  fHTimeBits->Sumw2() ;

  fHFineTime = new TH1F("fHFineTimeLAV","Fine Time; FT; Counts", 256, -0.5,255.5) ;

  fHTimeStamp16   = new TH1F("fHTimeStamp16LAV","Time Stamp below 4th bit;Time Stamp mod 16 ; Count", 16, -0.5,15.5) ;
  fHTimeStamp128  = new TH1F("fHTimeStamp128LAV","Time Stamp below 7th bit;Time Stamp mod 128 ; Count", 128, -0.5,127.5) ;
  fHTimeStamp1024 = new TH1F("fHTimeStamp1024LAV","Time Stamp below 10th bit;Time Stamp mod 1024 ; Count", 256, -0.5,1023.5) ;
  fHTimeStamp8192 = new TH1F("fHTimeStamp8192LAV","Time Stamp below 13th bit;Time Stamp mod 8192 ; Count", 512, -0.5,8191.5) ;

  fHFineTimeStamp128   = new TH1F("fHFineTimeStamp128LAV",
				 "Fine Time Stamp below 7th bit; Fine; Count", 128, -0.5,127.5) ;

  fHPrimFFT= new TH1D("fHPrimFFTLAV","FFT - LAV; Hz; ",65536,0.,10000.);
  fHPrimFFT->SetLineColor(kBlue);

  fHPrimTimeFFT = new TH1D("fHPrimTimeFFTLAV","LAV time profile for FFT; ns",65536,0.,6.5536E+9);
  ...
  TCanvas * cLAVPrimTS = AddCanvasTab("Rate");
  cLAVPrimTS->Divide(1,3);
  cLAVPrimTS->cd(1)->SetPad(.005, .500, .995, .995);
  cLAVPrimTS->cd(1);
  fHPrimTime->Draw();
  fPText->Draw();
  cLAVPrimTS->cd(2)->SetPad(.500, .005, .995, .495);
  cLAVPrimTS->cd(2);
  cLAVPrimTS->cd(2)->SetLogy(kTRUE);
  fHPrimDeltaTS->Draw();
  cLAVPrimTS->cd(3)->SetPad(.005, .005, .495, .495);
  cLAVPrimTS->cd(3);
  cLAVPrimTS->cd(3)->SetLogy(kTRUE);
  fHPrimDelta->Draw();

  TCanvas * cLAVZoom = AddCanvasTab("Burst structure");
  cLAVZoom->Divide(1,3);
  cLAVZoom->cd(1)->SetPad(.005, .500, .500, .995 );
  cLAVZoom->cd(1);
  fHPrimTimeZoom1->Draw();
  cLAVZoom->cd(2)->SetPad(.500, .500, .995, .995);
  cLAVZoom->cd(2);
  fHPrimFFT->Draw();
  cLAVZoom->cd(3)->SetPad(.005, .005, .995, .495);
  cLAVZoom->cd(3);
  fHPrimTimeZoom2->GetXaxis()->SetRangeUser(3000.,4000.);
  fHPrimTimeZoom2->Draw();

  TCanvas * cLAVPrimID = AddCanvasTab("Primitive IDs");
  cLAVPrimID->cd(1)->SetLogx(kTRUE);
  fHPrimID->Draw("hbar1");
  fHPrimID->SetFillColor(kGreen);
  fHPrimID->SetBarWidth(0.8);

  /*TCanvas * cLAVBits = */AddCanvasTab("Time Bits");
  fHTimeBits->GetYaxis()->SetRangeUser(0.4,0.6);
  fHTimeBits->Draw();

  TCanvas * cLAVTimeStamps = AddCanvasTab("Time Stamps");
  cLAVTimeStamps->Divide(2,2) ;
  cLAVTimeStamps->cd(1) ;
  fHTimeStamp16->Draw();
  cLAVTimeStamps->cd(2) ;
  fHTimeStamp128->Draw();
  ...
  /*TCanvas * cLAVFineTime = */AddCanvasTab("Fine Time");
  fHFineTime->Draw() ;

  /*TCanvas * cLAVFineTimeStamps = */AddCanvasTab("Fine Time Stamps");
...
void LAVPrimMon::Update(TTree* tree, TString TimeRunBurst, TString time) {
  ...
  cout << "LAV entries " << nEntries << endl;
  ...
  for (int iEnt = 0; iEnt < nEntries; iEnt++) {
    ...
    if(iEnt%100000==0){
      cout << "LAV: " << (100.*iEnt)/nEntries << "% of processing complete \r";
    }
  ...
  fHPrimTime->SetTitle(Form("LAV primitives rate - %s", TimeRunBurst.Data()));
  fHPrimTimeZoom1->SetTitle(Form("LAV - %s", TimeRunBurst.Data()));
...
void LAVPrimMon::Rescale(TH1F* h){
