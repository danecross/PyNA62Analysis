...
public:
  ...
  static SpectrometerParameters& GetInstance() {
    ...
    static SpectrometerParameters instance;
    return instance;
  }
...
private:
  ...
  TString fRTFileName = "";
  TString fXTFileName = "";
  Double_t fNullCoordinate = 0;
  Double_t fEC; ///< e*c
  Int_t fNChambers = 4;  ///< Total number of chambers.
  Int_t fNViews = 4; ///< Total number of views per chamber.
  Int_t fNPlanes = 4; ///< Total number of planes per view.
  Int_t fNStraws = 122; ///< Maximum number of straws per plane.
  Double_t fTWindowMin = 0; ///< Minimum T of accepted hits.
  Double_t fTWindowMax = 0; ///< Maximum T of accepted hits.
  Double_t fMaxHitPlane = 0; ///< Maximum hits per plane.
  Double_t fTDriftMax = 0;           ///< Maximum drift time.
  Double_t fTTrailingCut2Hits = 0;   ///< Trailing time vut for each 2-hit cluster.
  Double_t fTTrailingSigma3Hits = 0; ///< Sigma of the average trailing time of 3-hit clusters.
  Double_t fTTrailingSigma2Hits = 0; ///< Sigma of the average trailing time of 2-hit clusters.
  Double_t fRadiusSum3HitsMin = 0;   ///< Minimum value of the sum of the radius of the 3 hits in a 3-hit cluster.
  Double_t fRadiusSum3HitsMax = 0;   ///< Maximum value of the sum of the radius of the 3 hits in a 3-hit cluster.
  Double_t fRadiusSum2HitsMax = 0;   ///< Maximum value of the sum of the radius of the 3 hits in a 3-hit cluster.
  Double_t fDeltaStrawPositionMax = 0; ///< Maximum difference allowed in the LR algorithm between the straw position of the reference hit and the one of the next hit.
  Double_t fChi2Triplet = 0; ///< \f$\chi^{2}\f$ of the fitted triplets of tubes.
  Double_t fTripletParameter1 = 0; ///< Minimum radius defining the triplet region.
  Double_t fTripletParameter2 = 0; ///< Maximum radius defining the triplet region.
  Double_t fTripletParameter3 = 0; ///< Radius above which the radius assignment has the same sign in two compatible tubes of the same view.
  Double_t fTripletParameter4 = 0; ///< Tolerance for the previous parameter in case of 2-hits.
  Double_t fDoubletParameter1 = 0; ///< Number of sigma of the time chi2 cut for 2-hit cluster reconstruction.
  Double_t fDoubletParameter2 = 0; ///< Number of sigma of the sumwire chi2 cut for 2-hit cluster reconstruction.
  Double_t fDoubletParameter3 = 0; ///< Maximum chi2-like of sumwire+time for 2-hit cluster reconstruction (3-hit cluster-like).
  Double_t fPlaneStaggering = 0; ///< Staggering between two half-views. Used for the pattern recognition of triplets.
  Double_t fHitSlopeMax = 0; ///< Maximum track slope allowed from the reconstructed hits in one view.
  Double_t fNSigmaSlopeCut = 0; ///< Number of sigma on the slope to cut on the maximum slope.
  Double_t fInterTtrailingCut = 0; ///< Maximum T trailing difference between 2 clusters belonging to different views.
  Double_t fInterDistanceCut = 0; ///< Max distance between 2 intersections.
  Double_t fInterQuality4ViewsCut1 = 0; ///< Quality cut for 4 views intersection.
  Double_t fInterQuality4ViewsCut2 = 0; ///< Quality cut for 4 views intersection.
  Double_t fInterQuality4ViewsCut3 = 0; ///< Quality cut for 4 views intersection.
  Double_t fInterQuality4ViewsCut4 = 0; ///< Quality cut for 4 views intersection.
  Double_t fInter4ViewsTtrailingSigma = 0; ///< Average trailing time resolution in 4-view chamber-hits.
  Double_t fInterQuality3ViewsCutXYU = 0; ///<Quality cut for 3 views intersection (XYU).
  Double_t fInterQuality3ViewsCutXYV = 0; ///<Quality cut for 3 views intersection (XYV).
  Double_t fInterQuality3ViewsCutXUV = 0; ///<Quality cut for 3 views intersection (XUV).
  Double_t fInterQuality3ViewsCutYUV = 0; ///<Quality cut for 3 views intersection (YUV).
  Double_t fInter3ViewsTtrailingSigma = 0; ///< Average trailing time resolution in 3-view chamber-hits.
  Double_t fCombinationXCorridor0 = 0;
  Double_t fCombinationXCorridor1 = 0;
  Double_t fCombinationXCorridor2 = 0;
  Double_t fCombinationXCorridor3 = 0;
  Double_t fCombinationYCorridor0 = 0;
  Double_t fCombinationYCorridor1 = 0;
  Double_t fCombinationYCorridor2 = 0;
  Double_t fCombinationYCorridor3 = 0;
  Double_t f3HitsCombRadiusAtMC = 0;
  Double_t fMaxMomentum = 0; ///< Maximum momentum for combination reconstruction in TrackCollector.
  Double_t fMaxAngle = 0; ///< Maximum slope for combination reconstruction in TrackCollector.
  Double_t fCombQualityCut = 0;
  Double_t fCombHoughDeltaCut = 0;
  Double_t fGuessAverageSigma = 0; ///< Estimated average sigma on radius measurement.
  Double_t fGuessSingletSigma = 0; ///< Guessed sigma to test the hits in the Kalman filter for pattern recognition (clusters without slope measurement = 0; used in TrackCollector).
  Double_t fZRef = 0; ///< Longitudinal position of the reference plane for the final track fit.
  Double_t fXX0perPlane = 0; ///< Estimated X/X0 per single plane of straws.
  Double_t fCutDyFit[4] = {0, 0, 0, 0}; ///< Cut on difference between measured and fitted point per chamber.
  Int_t fHistoDebug = 0; ///< Flag to switch on and off the histograms for monitoring.
  Bool_t fIsMuonRun = false;
  Int_t fRTMode = 0;
  Int_t fXTMode = 0;

  // Timing offsets
  Double_t fMagicT0 = 0;
  Bool_t   fMagicT0ScanEnabled = false;
  ...
  Float_t fTimeStep = 0;
  Float_t fStrawT0 = 0;
  Float_t fNClustersPermm = 0;
  Float_t fDiscrSetTime = 0;
  Float_t fEdgeDeadTime = 0;
  Float_t fSameEdgeDeadTime = 0;
  Float_t fThreshold = 0;
  Float_t fGain = 0;
  Float_t fCARIOCASlope = 0;
  Float_t fEquivalentNoiseCharge = 0;
  Float_t fIonizationEnergy = 0;
  Float_t fNTotalPermm = 0;
  Bool_t fNoiseSimu = false;
  Bool_t fSavePulseShapes = false;
  Bool_t fNoBulkHits = false;
