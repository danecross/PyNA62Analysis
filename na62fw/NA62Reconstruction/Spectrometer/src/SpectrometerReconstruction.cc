...
SpectrometerReconstruction::SpectrometerReconstruction(TFile* HistoFile, TString ConfigFileName) :
  NA62VReconstruction(HistoFile, "Spectrometer", ConfigFileName),
...
/// \EndMemberDescr
  ...
  fSlimRecoEvent = new TSlimRecoSpectrometerEvent();
...
/// \EndMemberDescr
    ...
    for (Int_t jHit=0;jHit<fRecoEvent->GetNHits();jHit++)
    {
      ...
      fHit->SetLocalPosition(TVector3(straw->GetLocalXPosition(),-9999.,straw->GetGlobalZPosition()));
