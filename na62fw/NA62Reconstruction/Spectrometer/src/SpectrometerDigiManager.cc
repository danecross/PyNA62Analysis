...
/// \EndMemberDescr
  ...
  for (Int_t iDigi=0; iDigi<nDigis; iDigi++) { // Loop over leadings
    ...
    if (DriftTime >= kTWindowLeadMax || DriftTime <= kTWindowLeadMin) continue;
  ...
  for (Int_t jch=0; jch<8000; jch++) { // Loop over the channels
    ...
    if (MCHit) { // Use the RT dependence for MC according to the digitization algorithm used
      ...
      WireDistance = fPar.GetRTDependence(DriftTime);
        ...
        if (fPar.GetRTMode()){ // RT measured on data: to be debugged yet
          WireDistance = fPar.GetRTDependenceDataFull(
              (122*planeid+Digi->GetStrawID()+1), DriftTime, t0var);
        } else {
          WireDistance = fPar.GetRTDependenceData(DriftTime/1000.);
        }
  ...
  for (Int_t iDigi=0; iDigi<nDigis; iDigi++) {
    ...
    if (TrailingTime >= kTWindowTrailMax || TrailingTime<=kTWindowTrailMin) continue;
