  ...
  fHTimeCor(nullptr)
{
  ...
  fDMag = fGeo.GetMagnetZLength();
  fZMag = fGeo.GetMagnetZPosition();
  fEC = fPar.GetEC();
  fBMag = fGeo.GetMagnetFieldStrength()*1000;//0.6928Tm*1000
  fZRef_Upstream = 180000;//mm
  fZRef_Downstream = 222000;//mm
  fXX0 = fPar.GetXX0perPlane();
  fGuessAverageSigma = fPar.GetGuessAverageSigma();
  fGuessSigmaNoSlope = fPar.GetGuessSingletSigma();
  ...
  for (Int_t jj=0; jj<26; jj++) {
    for (Int_t kk=0; kk<26; kk++) {
      for (Int_t ll=0; ll<169; ll++) {
        fBxMNP33[ll][jj][kk] = fPar.GetBMNP33(jj,kk,ll).X();
        fByMNP33[ll][jj][kk] = fPar.GetBMNP33(jj,kk,ll).Y();
        fBzMNP33[ll][jj][kk] = fPar.GetBMNP33(jj,kk,ll).Z();
        if (jj==0 && kk==0) fZcoorMNP33[ll] = fPar.GetZMNP33(ll);
      }
    }
  } 
  for (Int_t jj=0; jj<17; jj++) {
    for (Int_t kk=0; kk<17; kk++) {
      for (Int_t ll=0; ll<30; ll++) {
        fBxFringe[ll][jj][kk] = fPar.GetBFringe(jj,kk,ll).X();
        fByFringe[ll][jj][kk] = fPar.GetBFringe(jj,kk,ll).Y();
        fBzFringe[ll][jj][kk] = fPar.GetBFringe(jj,kk,ll).Z();
        if (jj==0 && kk==0) fZcoorFringe[ll] = fPar.GetZFringe(ll);
      }
    }
  } 
...
void Track::FitFullMNP33Forward()
{
  ...
  Int_t jz = 0;
  while (jz<168) {
    Double_t zstop = fZcoorMNP33[jz+1]+fZMag;
    if (zstop<zstart) {
      jz++;
      continue;
    }
  ...
  jfz = 16;
  while (jfz<29) {
    Double_t zstop = fZcoorFringe[jfz+1]+fZMag;
...
void Track::Transport_magnet(Int_t jz, Double_t zstart, Double_t zstop, Bool_t bfringe)
{
  ...
  m22 = cosa;
  m23 = -sina*deltax;
  // following variables might be useful in future, suppress warnings
...
Double_t Track::chi2_calc(){
  ...
  for(Int_t i = 0; i<fNResidualHits; i++){
    ...
    if (fIteration==1 && fXTMode==1){
      fTimeCorX[i] = fPar.GetXTDependence(viewID,orthcoord);
    }
  ...
  for (Int_t jj=0; jj<4; jj++) {
    ...
    for (UInt_t jclust=0; jclust<thisChamberHit->GetNCluster(); jclust++) {
      ...
      for (UInt_t jhit=0; jhit<thisCluster->GetNHit(); jhit++) {
        ...
        Double_t xstraw = fGeo.GetStraw(thisHit->GetChannelID())->GetLocalXPosition();
        // Time signal propagation correction
////        if (fDATA && fIteration==2) {
////          Double_t newradius = fPar.GetRTDependenceData((thisHit->GetDriftTime()-fTimeCorX[jHitForFit])/1000.);
        if (fRTMode) {
            Double_t newradius = fPar.GetRTDependenceDataFull(thisHit->GetPlaneID()*122+thisHit->GetStrawID()+1,(thisHit->GetDriftTime()-fTimeCorX[jHitForFit]),(thisHit->GetEnergy()));
...
void Track::ExtractBMag(Double_t xmag, Double_t ymag, Double_t *bmag) {
  ...
  Double_t bymag1 = fPar.GetByIntegral(j);
  Double_t bymag2 = fPar.GetByIntegral(j+27);
  Double_t bymag3 = fPar.GetByIntegral(j+1);
  Double_t bymag4 = fPar.GetByIntegral(j+27+1);
  Double_t by1 = bymag1;         
...
//  bmag[0] = (by1+by2+by3+by4)/4.;
  ...
  Double_t bxmag1 = fPar.GetBxIntegral(j);
  Double_t bxmag2 = fPar.GetBxIntegral(j+27);
  Double_t bxmag3 = fPar.GetBxIntegral(j+1);
  Double_t bxmag4 = fPar.GetBxIntegral(j+27+1);
  Double_t bx1 = bxmag1;         
