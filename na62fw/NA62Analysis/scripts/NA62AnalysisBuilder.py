...
def prepareUserFolder(args):
    ...
    FWPath = getVar("ANALYSISFW_PATH", os.path.abspath(os.path.join(os.path.realpath(__file__), "../../")))
    NA62RECOSOURCE = getCheckVar("NA62RECOSOURCE")
...
def parseArgs():
    ...
    common_flags.add_argument('-j', '--jobs', default = 1, type = int, help = "Number of processes to use for building (same as make -j). Valid with all commands")
    common_flags.add_argument('--strict-order', default = False, action = "store_true", help = """Use the analyzer in the same order as defined
                                in the configuration file (beware of dependencies between analyzers). Valid with all commands""")
    common_flags.add_argument('--build-user-only', dest = "build_user_only", default = False, action = "store_true", help = "Compiles only the user directory. Does not check the FW state. Valid with all commands")
