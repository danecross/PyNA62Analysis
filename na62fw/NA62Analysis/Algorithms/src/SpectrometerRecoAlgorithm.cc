...
SpectrometerRecoAlgorithm::SpectrometerRecoAlgorithm (BaseAnalysis *ba, Analyzer* ana, const string &name) :
...
{
  if (!GetIsTree()) return;
  if (!GetEventHeader()) return; // run only on reconstructed files (output of NA62Reco)
