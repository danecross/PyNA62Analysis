...
(BaseAnalysis *ba, Analyzer* ana, const std::string &name) :
  ...
  AddParam("UseKalmanFilter",        &fUseKalmanFilter,       false); // Use Kalman filter instead of the default algorithm?
...
void GigaTrackerRecoAlgorithm::InitHistograms() {
  ...
  BookHisto("hChi2XY",   new TH1F("hChi2XY",";#chi^{2} XY;count",200,0,500));
  ...
  BookHisto("hProbChi2XY",new TH1F("hProbChi2XY",";Prob #chi^{2} XY 1 dof;count",1000,0,1)); // Filled in case of KF algorithm
	...
	for(i1=ii1.first; i1!=ii1.second; ++i1){
	  for(i2=ii2.first; i2!=ii2.second; ++i2){
	    ...
	    if (fFillHistograms) {
	      ...
	      if (fUseKalmanFilter) {
		FillHisto("hChi2XY", cand->GetChi2X());
		FillHisto("hProbChi2XY", TMath::Prob(cand->GetChi2X(),1));
	      }
	      else {
		FillHisto("hChi2X", cand->GetChi2X());
		FillHisto("hProbChi2X", TMath::Prob(cand->GetChi2X(),2));
	      }
...
void GigaTrackerRecoAlgorithm::InitCorrections() {
  ...
  TString Line;
  if (fRedoXYCorr==1) {
    for (UInt_t iS(0); iS<3; iS++) fPosOff[iS] = fParameters->GetGigaTrackerStationMisalignment(iS); // [mm]
  }
...
void GigaTrackerRecoAlgorithm::BuildCandidate(TRecoGigaTrackerCandidate* cand) {
  ...
  const Double_t X0Silicon(93.7); // [mm]
  ...
  for (UInt_t iS=0; iS<3; iS++) {
    ...
    Y[iS] = cand->GetPosition(iS).Y();
    T[iS] = cand->GetTimeStation(iS); // ns
  }

  Double_t DeltaTrim[3]; // [mm]
  for (UInt_t iS=0; iS<3; iS++) {
    DeltaTrim[iS] = fParameters->GetGigaTrackerStationPositionCorrected(iS).Z() -              
      fParameters->GetGigaTrackerMDXMagnetPosition().Z(); //hot e-3
 }

  // cout << "Delta12: " << Delta12 << endl;
  // cout << "Delta23: " << Delta23 << endl;
  // cout << "DeltaBend: "<< DeltaBend << endl;
  // cout << "DeltaTrim[2]: " << DeltaTrim[2] <<endl;

  // cout << "BLbend: " << BLbend << endl;
  // cout << "BLtrim: " << BLtrim << endl;
  ...
  Double_t sigmaX[3];
  for (UInt_t iS=0; iS<3; iS++) { // begin of loop on iS
    // 1-pixel hit
    if ( nPixels[iS]==1 ) {
  ...
  Double_t GTKthickness[3] = {0.,0.,0.}; // thickness for GTK2 and GTK3 = Z_chip + Z_sensor + Z_cooling_plate
  Double_t theta[2];
  ...
  //Kalman Filter
  if (fUseKalmanFilter) {
    Double_t chi2XY_KF = 0.0;

    //Initialize Kalman Filter

    //Propagation
    ROOT::Math::SMatrix<double,5,5> F0; //propagate to GTK1
    ROOT::Math::SMatrix<double,5,5> F1; //propagate to GTK2
    ROOT::Math::SMatrix<double,5,5> F2; //propagate to GTK3
    ROOT::Math::SMatrix<double,2,5> H;
    ROOT::Math::SMatrix<double,5,5> Q;  //process Noise Covariance matrix.
    ROOT::Math::SMatrix<double,5,5> C0;
    ROOT::Math::SMatrix<double,2,2> V;  //covariance matrix of measurement. 

    // No magnetic field. Just propagete to the GTK1. Identity matrix
    F0 = ROOT::Math::SMatrixIdentity();

    Double_t Delta[2] = {Delta12, Delta23};
    
    //Propagate to GTK2
    F1 = ROOT::Math::SMatrixIdentity();
    F1(2,0) = Delta12; // [mm]
    F1(3,1) = Delta12; // [mm]
    F1(3,4) = - 1e-9 * fClight * BLbend * DeltaBend;; // [mm * (MeV/c)]
    
    // cout << "F1(1,4): " << F1(1,4) << endl;
    
    //Propagate to GTK3
    F2 = ROOT::Math::SMatrixIdentity();
    F2(2,0) = Delta23;
    F2(2,4) = - 1e-9 * fClight * BLtrim * DeltaTrim[2];// [mm * (MeV/c)]
    F2(3,1) = Delta23;
    F2(3,4) = + 1e-9 * fClight * BLbend * DeltaBend;;  // [mm * (MeV/c)]
    F2(0,4) = - 1e-9 * fClight * BLtrim; // [MeV/c]
    
    // cout << "DeltaTrim[2]: " << DeltaTrim[2] << "\n" << "BLtrim: " << BLtrim << "\n"  << 1e-9 * fClight * BLtrim << endl; 
    // cout << "F2(0,4): " << F2(0,4) << endl;
    // cout << "F2(1,4): " << F2(1,4) << endl;
    // cout << "F2(2,4): " << F2(2,4) << endl;

    //Measurement adapation matrix. We measure only x and y so is a 2X2 identiy matrix and the rest of the matrix is 0
    H(0,2)= 1;
    H(1,3)= 1;

    //covariance matrix of measurement. pitch uncertainty.
    for(UInt_t i =0; i<2; i++) V(i,i)= sigmaX[0]*sigmaX[0];  

    //Initial state. Assume the beam is perfectly centered and with p=pz =75 GeV. The rest is set to 0;
    ROOT::Math::SVector<double,5 >  X0; 
    X0(2) = cand->GetPosition(0).X();
    X0(3) = cand->GetPosition(0).Y(); 
    X0(4)= 1/75e3; // c/MeV
    

    //state covariance matrix. 
    C0(0,0) = TMath::Power(5*0.11e-3,2); // mrad^2 
    C0(1,1) = TMath::Power(5*0.11e-3,2); // mrad^2
    C0(2,2) = TMath::Power(3*0.3,2)/12.; // mm^2 
    C0(3,3) = TMath::Power(3*0.3,2)/12.; // mm^2
    C0(4,4) = TMath::Power(5*0.01*1./75.e3,2); // (c/MeV)^2

    /* Create measure vector, and store measure value */
    ROOT::Math::SVector<double,2 > m;

    /* Create The Filter */
    KalmanFilter kalmanFilter(false);
    kalmanFilter.setVerbose(false);

    /* Initialize the Filter */
    kalmanFilter.setMatrices(F0, H, Q, V);
    kalmanFilter.setInitial(X0, C0);
  
    for (UInt_t iStep=0; iStep<3; ++iStep) {
      if(iStep > 0){

	//covariance matrix of measurement. pitch uncertainty.
	for(UInt_t i =0; i<2; i++) V(i,i)= sigmaX[iStep]*sigmaX[iStep]; 

	if ( iStep == 1 ) {
	  kalmanFilter.setF(F1);
	  kalmanFilter.setV(V);  
	  }
	else if ( iStep == 2 ) {
	    kalmanFilter.setV(V);
	    kalmanFilter.setF(F2);
	}
	else {
	  cout << "[GigaTrackerRecoAlgorithm] MORE THAN TWO STEPS! " << endl;
	}

	// NIM A329 (1993) 493-500  
	Double_t thetaKF =  kalmanFilter.X.back()[4]*13.6 * TMath::Sqrt(GTKthickness[iStep-1]/X0Silicon) *(1 + 0.038 * TMath::Log(GTKthickness[iStep-1]/X0Silicon));	  

	//cout<<"theta "<<theta<<" kalmanFilter.X.back()[4] "<<kalmanFilter.X.back()[4]<<" GTKthickness[iStep-1] "<<GTKthickness[iStep-1]<<" X0Silicon "<<X0Silicon<<endl;
	  
	Double_t dxdz = kalmanFilter.X.back()[0];
	Double_t dydz = kalmanFilter.X.back()[1];

	Double_t P3P3 = TMath::Power(thetaKF,2)*(1+dxdz*dxdz+dydz*dydz)*(1.+dxdz*dxdz);
	Double_t P3P4 = TMath::Power(thetaKF,2)*(1+dxdz*dxdz+dydz*dydz)*dxdz*dydz;
	Double_t P4P4 = TMath::Power(thetaKF,2)*(1+dxdz*dxdz+dydz*dydz)*(1.+dydz*dydz);

	// diagonal
	Q(2,2) = P3P3*Delta[iStep-1]*Delta[iStep-1];
	Q(3,3) = P4P4*Delta[iStep-1]*Delta[iStep-1];
	Q(0,0) = P3P3;
	Q(1,1) = P4P4;

	// off-diagonal
	Q(2,3) = P3P4*Delta[iStep-1]*Delta[iStep-1];
	Q(3,2) = Q(2,3);
	Q(2,0) = P3P3*Delta[iStep-1];
	Q(0,2) = Q(2,0);
	Q(2,1) = P3P4*Delta[iStep-1];
	Q(1,2) = Q(2,1);
	Q(3,0) = P3P4*Delta[iStep-1];
	Q(0,3) = Q(3,0);
	Q(3,1) = P4P4*Delta[iStep-1];
	Q(1,3) = Q(3,1);
	Q(0,1) = P3P4;
	Q(1,0) = Q(0,1);
	...
	kalmanFilter.setQ(Q);	  
      ...
      m(0) = X[iStep];
      m(1) = Y[iStep];
      if(iStep==1) m(1) += -60.; // -60 mm shift in GTK2
      kalmanFilter.filter( m );

      if (fFillHistograms) {
	FillHisto(Form("hResX_Sta%d",iStep), kalmanFilter.rFilt(2));
	FillHisto(Form("hNormResX_Sta%d",iStep),kalmanFilter.rFilt(2)/kalmanFilter.CFilt.back()(2,2));
	FillHisto(Form("hResY_Sta%d",iStep), kalmanFilter.rFilt(3));
	FillHisto(Form("hNormResY_Sta%d",iStep),kalmanFilter.rFilt(3)/kalmanFilter.CFilt.back()(3,3));
      }
    } // end of KF prediction loop (GTK1->GTK2->GTK3)

    for (UInt_t iStep=0; iStep<3; ++iStep)
      chi2XY_KF += kalmanFilter.chi2filt.at(iStep);

    //Calculate standard momentum components
    Float_t Pz =  (1/kalmanFilter.X.back()[4])/(TMath::Sqrt( 1+TMath::Power(kalmanFilter.X.back()[0],2) + TMath::Power(kalmanFilter.X.back()[1],2) ));
    Float_t Px  = kalmanFilter.X.back()[0]*Pz;
    Float_t Py  = kalmanFilter.X.back()[1]*Pz;

    Momentum.SetXYZ(Px, Py, Pz); // [MeV/c]
    chi2 = chi2XY_KF + chi2Time;
    cand->SetChi2X(chi2XY_KF);    

    //Covariance matrix setting
    for(UInt_t i=0; i<5;i++)
      for(UInt_t j=0; j<5;j++)
	cand->SetCovariance(i,j,kalmanFilter.CFilt.back()(i,j));

    //set GTK3 position according to KalmanFilter
    TVector3 pos(kalmanFilter.X.back()[2],kalmanFilter.X.back()[3],cand->GetPosition(2).Pz());
    cand->SetPosition(2,pos);
  }
  else {
    Double_t alpha =
      (fParameters->GetGigaTrackerStationPositionRaw(1).Z() - 
