...
void LAVT0::GenerateUserPDFReport(){
  if (!fSlewingFound) {
    std::cout << user_normal() << "cannot find one of the input histograms " << fDeltaLAV[0] << " " << fDeltaLAV[1] << std::endl;
    return;
  }
...
void LAVT0::RequestUserHistograms(){
  ...
  for (Int_t i=0; i<2; i++) {
    fDeltaLAV[i] = static_cast<TH2F*>(RequestHistogram(fDirName, referenceName[i], true));
...
void LAVT0::evaluateGlobalTZeroFit(Int_t referenceTime = 1, Double_t sigmaTime = 2., Int_t nSigma = 6, Int_t rebinX = 2, Int_t rebinY = 2){
  ...
  Double_t xRangeMin=0.0, xRangeMax=100.0, yRangeMin=-15.0, yRangeMax=15.0;
  if (referenceTime == 0) {
    ...
    std::cout << user_normal() << "wrong reference time in input: 0/1 for LowFromToT/HighFromSlope" << std::endl;
    return;
  }
  ...
  TGraphErrors* grafo = correctSlewing(referenceTime, &fDeltaLAV[referenceTime], 100, sigmaTime, nSigma, rebinX, rebinY,
  ...
  std::cout << user_normal() << "plot of slewing correction " << grafo;
  if(CanPrint()){
    if (grafo) {std::cout << " with " << grafo->GetN() << " points " << std::endl;}
    else { std::cout << " with 0 points -- plot will not be made " << std::endl;}
  }
...
TGraphErrors* LAVT0::correctSlewing(Int_t channel, TH2F** DeltaLAV, Int_t eventLimit, Double_t sigmaTime, Int_t nSigma, Int_t rebinX, Int_t rebinY,
  ...
  TH2F* newo = (TH2F*) dtlav->Rebin2D(rebinX,rebinY);
  ...
  for (Int_t i=0; i<NChannels; i++) {
    ...
    if (maxPopulationCh > 10) {
      ...
      TF1* func = new TF1("fitFunction",fFunctions,&LAVFitFunctions::EvaluateFitf,windowTMin,windowTMax,3,"LAVFitFunctions","EvaluateFitf");
      ...
      proj->Draw("");
      proj->Fit("fitFunction","QL","same",fitTMin,fitTMax);
...
//  }
  ...
  if (n_points > 5) {
    ...
    gr_Mean->Fit("fitFunction2","Q","0",Tav[nptsMin],Tav[nptsLimit]+1);
    gr_Mean->Fit("fitFunction2","QE","0",Tav[nptsMin],Tav[nptsLimit]+1);
    ...
    *coeff3      = func2->GetParameter(3); // unused: added to suppess a compulation warning [EG 15/05/16]
    *coeff3Err   = func2->GetParError(3);
