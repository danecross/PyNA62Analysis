...
UpstreamPileupGenerator::UpstreamPileupGenerator(NA62Analysis::Core::BaseAnalysis *ba) :
  ...
  fTimeWindowWidth(-1), fCedarTimeResponse(nullptr), fKaonGenerated(false),
  fRandom(nullptr), fCedarEvent(nullptr), fGTKEvent(nullptr) {
  ...
  AddParam("GenerateInTimePion",   &fGenerateInTimePion, 0); // 1=yes, 0=no
  AddParam("InTimePionTimeWindow", &fInTimePionTimeWindow, 1.0); // half time window; negative value = exactly in time
...
void UpstreamPileupGenerator::Process(Int_t) {
  ...
  if (!fGenerateInTimePion) { // standard mode
    ...
    // Generate exactly one in-time pion:
    // the ID of the track taken from the library is determined
    // by the value of the fGenerateInTimePion parameter.
    // The pion could be either exactly in time,
    // or generated uniformly in time within a userp-specified window.
    Double_t TrackTime = GetEventHeader()->GetFineTime()*TdcCalib;
    if (fInTimePionTimeWindow>0.0) {
      TrackTime += 2.0 * fInTimePionTimeWindow * (fRandom->Rndm()-0.5);
    }
    Int_t TrackID = fRandom->Integer(fHitLibrary.size()) + fGenerateInTimePion;
    TrackID = TrackID % fHitLibrary.size();
    AddTrack(TrackTime, 1, TrackID);
  }
...
void UpstreamPileupGenerator::AddTrack(Double_t Time, Int_t particleID, Int_t trackID) {
  ...
  if (particleID==0){ // kaon
    ...
    CedarMeanNHits = fCedarMeanNHitsPerPion;
  }
