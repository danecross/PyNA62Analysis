...
void SpectrometerGigaTrackerMatching::Process(int iEvent){
  ...
  for(Int_t I=0;I<Nloop;I++){ //loop over spectrometer tracks
    ...
    if(fMatchingMode == 1){ //for K3pi (Testing Mode)
      ...
      if(BestDisc !=	1000000.0 ) {
          FillHisto("DiscriminantPlots/hBestCandDiscriminant",BestDisc); //The if statement ensures hist is not filled if there is no GTK candidate
          FillHisto("DiscriminantPlots/hBestCandDiscriminant_Full",BestDisc);
      }
    ...
    for(Int_t j = 0 ; j<NTracksToMatch ; j++){ //loop over the number of downstream tracks
      ...
      for(Int_t i = 0 ; i<nRecoCand ; i++){ //loop over GTK Candidate
        ...
        CDAToolA->SetLine1PointDir(A0,a); //Position of K GTKCand at GTK3; unit vector for GTK K cand direction
        CDAToolA->SetLine2PointDir(B0,b);
