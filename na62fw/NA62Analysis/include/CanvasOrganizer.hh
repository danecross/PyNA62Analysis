...
/// \EndBrief
union plotType_u{
	plotType_u(): histo(nullptr) {
		/// \MemberDescr
		/// Default constructor
		/// \EndMemberDescr
	}
	plotType_u(TH1* h): histo(h) {
		/// \MemberDescr
		/// \param h: pointer to histogram
		///
		/// Constructor for TH1 derived
		/// \EndMemberDescr
	}
	plotType_u(TGraph* h): graph(h) {
		/// \MemberDescr
		/// \param h: pointer to histogram
		///
	...
	plotType_u(TEfficiency* h): efficiency(h) {
		/// \MemberDescr
		/// \param h: pointer to histogram
		///
		/// Constructor for TEfficiency
		/// \EndMemberDescr
	}

	TH1* histo;    ///< Pointer to TH1 derived
	TGraph* graph; ///< Pointer to TGraph derived
	TEfficiency* efficiency; ///< Pointer to TEfficiency derived
};

/// Type of Plot in the container
enum plotType_e {TTUNKNOWN, TTH1, TTH2, TTH3, TTGraph, TTEfficiency};
...
class CanvasOrganizer{
public:
	explicit CanvasOrganizer(TString name);
	...
	void AddHisto(TEfficiency* histoPtr, int row=-1, int col=-1);

	void AddHisto(std::vector<TH1*> histoPtr, int row=-1, int col=-1);
	void AddHisto(std::vector<TGraph*> histoPtr, int row=-1, int col=-1);
	void AddHisto(std::vector<TEfficiency*> histoPtr, int row=-1, int col=-1);
...
private:
	...
	/// \EndBrief
	struct plot_t{
		std::vector<plotType_u> ptr; ///< Pointer to the plot
	...
	int GetPlotIndexAt(int row, int col);
