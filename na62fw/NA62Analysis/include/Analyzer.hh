...
public:
	...
	explicit Analyzer(Core::BaseAnalysis* ba);
	Analyzer(Core::BaseAnalysis* ba, const std::string &name);
	...
	void EndOfBurst();
	void StartOfRun();
	void EndOfRun(bool disableSave);
    void EndOfJob(bool disableSave);
	void ProcessSpecialTrigger(int iEvent);
	void PreProcess();
	...
	virtual void StartOfBurstUser() {
		/// \MemberDescr Executed at start of burst. No default action to do.
		/// \EndMemberDescr
	};
	virtual void EndOfBurstUser() {
		/// \MemberDescr Executed at end of burst. No default action to do.
		/// Be careful, this is called after the event/file already changed
		/// \EndMemberDescr
	};
	virtual void StartOfRunUser() {
		/// \MemberDescr Executed at start of run. No default action to do.
		/// \EndMemberDescr
	};
	virtual void EndOfRunUser() {
		/// \MemberDescr Executed at end of run. No default action to do.
		/// \EndMemberDescr
	};
	virtual void EndOfJobUser() {
		/// \MemberDescr Executed at end of job. No default action to do.
		/// \EndMemberDescr
	};
	virtual void ProcessSOBEvent() {
		/// \MemberDescr Executed for the SOB event in the SpecialTrigger. No default action to do.
		/// \EndMemberDescr
	};
	virtual void ProcessEOBEvent() {
		/// \MemberDescr Executed for the EOB event in the SpecialTrigger. No default action to do.
		/// \EndMemberDescr
	};
	virtual void ProcessSpecialTriggerUser(int, unsigned int) {
		/// \MemberDescr
		...
		///
		/// Executed for each SpecialTrigger event. No default action to do.
		/// \EndMemberDescr
	};
	...
	void PrintParameters(bool force=false) const;
	...
	void AddBranch(TString name, TString branchName, T *pointer) {
		...
		if (fOutTree.count(name) == 0) {
			std::cout << normal() << "Output TTree " << name
	...
	void AddParam(TString name, TString type, T* address, T defaultValue) {
		...
		/// \EndMemberDescr
		if (!CheckType(type)) {
			std::cout << normal() << "Error when adding parameter " << name
					<< " : type not supported " << type << std::endl;
			return;
		}
		*address = defaultValue;
		fParams.insert(std::pair<TString, param_t>(name, param_t(type, address)));
	}
	...
	void AddParam(TString name, std::string *address, const std::string &defaultValue);
	...
	AnalyzerIdentifier GetIdentifier() const;
