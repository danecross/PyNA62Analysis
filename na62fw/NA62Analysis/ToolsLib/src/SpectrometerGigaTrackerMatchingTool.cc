...
(TRecoGigaTrackerEvent *event, TRecoSpectrometerCandidate *Scand, Double_t RefTime, Int_t RefDet) {
  ...
  if (RefDet!=kCedar && RefDet!=kCHOD && RefDet!=kRICH) {
    RefDet = kCedar;
    cout << "[SpectrometerGigaTrackerMatchingTool] Warning: invalid reference detector, Cedar assumed" << endl;
  }
...
(TRecoGigaTrackerCandidate *Gcand, TRecoSpectrometerCandidate *Scand) {
  ...
  fCDAcomp->SetLine1PointDir(Gcand->GetPosition(2), fBeamMomentumTemp); // position at GTK3
  fCDAcomp->SetLine2PointDir(Scand->GetPositionBeforeMagnet(), fTrackMomentumTemp);
  ...
  fCDAcomp->SetLine1PointDir(pos1, fBeamMomentumTemp);
  fCDAcomp->SetLine2PointDir(pos2, fTrackMomentumTemp);
  fCDAcomp->ComputeVertexCDA();
  fVertexTemp = fCDAcomp->GetVertex();
  fCDATemp = fCDAcomp->GetCDA();
}
