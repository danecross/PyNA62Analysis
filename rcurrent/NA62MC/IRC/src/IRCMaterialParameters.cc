...
IRCMaterialParameters* IRCMaterialParameters::fInstance = 0;

IRCMaterialParameters::IRCMaterialParameters()
{
...
IRCMaterialParameters* IRCMaterialParameters::GetInstance()
{
  if ( fInstance == 0 ) { fInstance = new IRCMaterialParameters(); }
  return fInstance;
}

void IRCMaterialParameters::DefineMaterials()
{
  ...
  G4Material* StainlessSteel = new G4Material("IRC_StainlessSteel",7.88*g/cm3,4); //Prepend subdetector name
  ...
  // Define 96% Pb + 4% Sn for IRC absorber
  // Density calculated to be: 0.04 * 7.365 + 0.96 * 11.34 = 11.181 g/cm3
  G4Material* PbSnAlloy = new G4Material("IRC_PbSnAlloy",11.181*g/cm3,2);
...
void IRCMaterialParameters::SetMaterialProperties()
{
  // Use here the material property tables prepared
}

TObjArray IRCMaterialParameters::GetHashTable()
{
  TObjArray IRCMaterialParameters;
  ...
  IRCMaterialParameters.Add(new DetectorParameter("fMaterialPropertiesNEntries",Value.Data(),
					       "Material Properties N Entries", ParameterData));
  ParameterData.Clear();


  return IRCMaterialParameters;
}

void IRCMaterialParameters::Print(){
