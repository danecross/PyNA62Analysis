...
PrimaryGeneratorMessenger::PrimaryGeneratorMessenger(PrimaryGeneratorAction* Action) :
  ...
  fTurtleDatacardCmd     = new G4UIcmdWithAString("/beam/DatacardName", this);
...
void PrimaryGeneratorMessenger::SetNewValue(G4UIcommand* command, G4String val) {
  G4cout << command->GetCommandPath() << " " << val << G4endl;
  if (command == fBeamCmd) { // beam type
  ...
  else if (command == fTurtleParticleNameCmd) { // Particle type in the turtle beam
    fAction->SetParticleName(val);
  }
  else if (command == fTurtleDatacardCmd) { // Turtle datacard (in case he used want to override the standard ones)
    fAction->SetDatacardName(val);
  }
  else if (command == fReplayFileCmd) {
    fAction->SetInputFileName(val);
  }
  else if (command == fExternalParticlesFileCmd) { // external file with the full path
    fAction->SetInputExternalParticlesFileName(val);
  }
  else if (command == fG4BeamStartZPositionCmd) { // Beam at Z
    fAction->SetG4BeamStartZPosition(val);
  }
  else if (command == fG4BeamLineInputFileCmd) { // use G4 beamline output as input
