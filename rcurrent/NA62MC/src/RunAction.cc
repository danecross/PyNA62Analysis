...
void RunAction::BeginOfRunAction(const G4Run* aRun) {
  ...
  // For some parameters, the protection is implemented in DatacardMessenger.
  ...
  if (DatacardManager::GetInstance()->GetDecayForce()) {
    ...
    if (DatacardManager::GetInstance()->GetRadiativePhotonMinEnergy() > DatacardManager::GetInstance()->GetRadiativePhotonMinEnergy()){
      G4cout << "ERROR: /decay/RadiativePhotonMinEnergy > decay/RadiativePhotonMaxEnergy : wrong radiative photon energy range" << G4endl;
    }
