...
void MUV1Scintillator::CreateGeometry() {
	...
	G4Box* scintillator_box_AirGap = new G4Box("MUV1scintillator_box_AirGap", (fScintillatorSize.x()-fSkinWidth),  (fScintillatorSize.y()-fSkinWidth),  (fScintillatorSize.z()-fSkinWidth) ); //todo: don't forget to remove 0.75*

	//This is the core box (scint) of the scintillator
	G4Box* scintillator_box_core = new G4Box("MUV1scintillator_box_core",  fBareScintillatorSize.x(),  fBareScintillatorSize.y(),  fBareScintillatorSize.z()); //todo: dont forget to remove ./2
	...
	if (fboolOpp && fLogical > 0) {
		...
		G4double cutBoxTollerance = 1 * mm ;
		G4double cutBoxSize       = sqrt(2.) * (44 + 2. * cutBoxTollerance) * mm; // cutBoxTollerance is added to avoid some round-off effects
	...
	if(fLogical==2  || fLogical == 4)	{
		groovePositionL[1] = ((fLogical == 2) ? 1 : -1) * (0.5 * fCutDepthLong  + 0.25 * fGrooveWidth * deltaLengthSign);
		groovePositionR[1] = ((fLogical == 2) ? 1 : -1) * (0.5 * fCutDepthShort + 0.25 * fGrooveWidth * deltaLengthSign);
	}
	else if(fLogical==1  || fLogical == 3)	{
		groovePositionL[1] = ((fLogical == 1) ? 1 : -1) * (0.5 * fCutDepthShort + 0.25 * fGrooveWidth * deltaLengthSign);
		groovePositionR[1] = ((fLogical == 1) ? 1 : -1) * (0.5 * fCutDepthLong  + 0.25 * fGrooveWidth * deltaLengthSign);
	}
	...
	if(fLogical==2  || fLogical == 4)	{
		grooveLengthL = 2 * fBareScintillatorSize.y() - fCutDepthLong  + 0.5 * fGrooveWidth * deltaLengthSign;
		grooveLengthR = 2 * fBareScintillatorSize.y() - fCutDepthShort + 0.5 * fGrooveWidth * deltaLengthSign;
	}
	else if(fLogical==1  || fLogical == 3)	{
		grooveLengthL = 2 * fBareScintillatorSize.y() - fCutDepthShort + 0.5 * fGrooveWidth * deltaLengthSign;
		grooveLengthR = 2 * fBareScintillatorSize.y() - fCutDepthLong  + 0.5 * fGrooveWidth * deltaLengthSign;
	}
