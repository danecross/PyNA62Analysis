...
LAVPbGlBlock::LAVPbGlBlock(G4int blockId):LAVVPbGlBlock()
{
  ...
  new G4PVPlacement(0,G4ThreeVector(0.,0.,0.),logicPbGlBlock,"LAV_PbGlBlock",fLogicalVolume,false,0);

  // Position of lightguide cylinder
  G4ThreeVector posLightGuide = pbglCenterOfBackFace+G4ThreeVector(0.,0.,-0.5*fLightGuideZLen);
  new G4PVPlacement(0,posLightGuide,logicLightGuide,"LAV_PbGlLightGuide",fLogicalVolume,false,0);

  // Position of steel slab (already computed)
  new G4PVPlacement(0,posSteelSlab,logicSteelSlab,"LAV_SteelSlab",fLogicalVolume,false,0);

  // Position of full u-metal cylinder
  G4ThreeVector posMuMetal = pbglCenterOfBackFace+G4ThreeVector(0.,0.,-0.5*fMuMetalZLen);
  new G4PVPlacement(0,posMuMetal,logicMuMetal,"LAV_MuMetal",fLogicalVolume,false,0);
  ...
  // Mu-Metal is black
  G4VisAttributes* MuMetalVisAtt = new G4VisAttributes(G4Colour(0.,0.,0.));
