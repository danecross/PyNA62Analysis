...
void CHANTIReconstruction::ParseConfFile (TString ConfFileName) {
  ...
  while (Line.ReadLine(NA62ConditionsService::GetInstance()->Get(fSignalParametersFileName))) {
    if (Line.BeginsWith("#")) continue;
      ...
      TObjArray *l = Line.Tokenize(" ");
      fTHRH = static_cast<TObjString*>(l->At(1))->GetString().Atof();
      delete l;
      continue;
    }
      ...
      if (CCand) {
        ...
        if(!((X>-95./2 && X<95./2) && (Y>-65./2 && Y<65./2))){
          ...
          if(iPlane>= 0 && iPlane<6)
            fHXYClusterPerPlane[iPlane]->Fill(chanti_hit_corrected_distance_to_fiber(CCand->GetXPos()), chanti_hit_corrected_distance_to_fiber(CCand->GetYPos()), 1.0/CCand->GetXYMult());
          else
...
void CHANTIReconstruction::FillTimes(Double_t ReferenceTime) {
  ...
  for (Int_t iCand=0; iCand<fRecoEvent->GetNCandidates(); iCand++) {
    fCandidate = static_cast<TRecoCHANTICandidate*>(fRecoEvent->GetCandidate(iCand));
    //Channel Distance From SiPM Occupancy
    Int_t NumberOfGhost = fCandidate->GetXYMult();
    if(NumberOfGhost ==1){
      Int_t CandNHit = fCandidate->GetNHits();
      for (int CandHitIndex=0; CandHitIndex < CandNHit; CandHitIndex++){
	TRecoCHANTIHit* CCandRecoHit = static_cast<TRecoCHANTIHit*>(fCandidate->GetHit(CandHitIndex));
	...
	if (fChannelHistograms)	(static_cast<CHANTIChannel*>(fChannels[ROCH])->FillPosition(fCandidate->GetXPos(),fCandidate->GetYPos(),Time, ReferenceTime));
      }
    }
...
void CHANTIReconstruction::InitHistograms()
{
  ...
  for(int iPlane = 0; iPlane < fNPlanes; iPlane++){
    ...
    fHXYClusterPerPlane[iPlane] = new TH2F(Form("XYClusterInPlane%d",iPlane+1),Form("XYCluster In Plane %d",iPlane+1),200,-fStep*10,fStep*10,200,-fStep*10,fStep*10);
