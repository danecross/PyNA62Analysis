...
void SAVReconstruction::ParseConfFile(TString ConfFileName) {
  ...
  while (Line.ReadLine(confFile)) {
    ...
    else if (Line.BeginsWith("RunNumber")){
      fRunNumber = TString(Line(TRegexp("[0-9]+"))).Atoi();
      continue;
    }
    ...
    for (UInt_t  iPeak=0; iPeak<PeakPosition.size(); iPeak++){
		...
		if (PeakPosition.size()>1 && iPeak<(PeakPosition.size()-1) && (PeakPosition[iPeak+1] - PeakPosition[iPeak])< 8) {
			...
			if (peak1>0. && peak1 < 3.e+4) {
				...
				if (DetectorID==0) peak1 *= fSACchannelEq[Channel];

				// SAC energy.
				if (DetectorID==0) energy1 = peak1 / fSAVcalibration[1][0];
			...
			if (peak2>0. && peak2 < 3.e+4) {
				...
				// SAC channel equalization.
				if (DetectorID==0) peak2 *= fSACchannelEq[Channel];

				// SAC energy.
				if (DetectorID==0) energy2 = peak2 / fSAVcalibration[1][0];
		...
		//---------------------------------
			...
			if (peak>0. && peak < 3.e+4) {
				...
				// SAC channel equalization.
				if (DetectorID==0) peak *= fSACchannelEq[Channel];

				// SAC energy.
				if (DetectorID==0) energy = peak / fSAVcalibration[1][0];
