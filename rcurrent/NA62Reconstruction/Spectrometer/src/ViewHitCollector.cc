...
/// \EndMemberDescr
  ...
  for (Int_t jPlane=0;jPlane<4;jPlane++) { // Plane Loop
    ...
    for (Int_t jHit=0;jHit<GetPlane(jPlane)->GetN();jHit++) { // Hit Loop
      ...
      Straw *fStraw = fGeo->GetStraw(fHit->GetChannelID());
      if(!fStraw) continue;
      Double_t fStrawX = fStraw->GetLocalPosition().X();
      Double_t fStrawZ = fStraw->GetPosition().Z();
      ...
      for (Int_t jPlaneNext=jPlane+1; jPlaneNext<4;jPlaneNext++) { // Loop on hits of the next plane (pair search)
        for (Int_t jHitNext=0; jHitNext<GetPlane(jPlaneNext)->GetN(); jHitNext++) {
          ...
          Straw *fStrawNext = fGeo->GetStraw(fHitNext->GetChannelID());
          if(!fStrawNext) continue;
          Double_t fStrawNextX = fStrawNext->GetLocalPosition().X();
          Double_t fStrawNextZ = fStrawNext->GetPosition().Z();
          Double_t deltaStrawX = fStrawX-fStrawNextX;
          ...
          for (Int_t jPlaneNextToNext=jPlaneNext+1; jPlaneNextToNext<4; jPlaneNextToNext++) {  // Loop on hits of the next to next plane (triplet search)
            for (Int_t jHitNextToNext=0;jHitNextToNext<GetPlane(jPlaneNextToNext)->GetN();jHitNextToNext++) {
              ...
              if (fabs(fStrawNextToNext->GetLocalPosition().X()-fStrawNextX)>fPar->GetDeltaStrawPositionMax()) continue;
              ...
              Straw *strawArray[3] = {fStraw,fStrawNext,fStrawNextToNext};
          ...
          if (matchHitId>-1) {
            ...
            AddHit(fHit,fPos,fStrawZ,fViewAngle,fHitId);
            GetPlane(jPlane)->SetHitFlag(jHit,3);
            AddHit(fHitNext,fPosNext,fStrawNextZ,fViewAngle,fHitNextId);
            GetPlane(jPlaneNext)->SetHitFlag(jHitNext,3);
            AddHit(matchHit,fPosNextToNext,matchStraw->GetPosition().Z(),fViewAngle,matchHitId);
          ...
          if (matchHitId==-1) {
            ...
            if (Pairing(hitWireDistance,hitPlanes,fChamberID)) {
              fPos = fStrawX+isign*sign*hitWireDistance[0];
              fPosNext = fStrawNextX-isign*sign*hitWireDistance[1];
              if (fabs(deltaStrawX-isign*fPar->GetPlaneStaggering())<1) {
                if (hitWireDistance[1]>fPar->GetTripletParameter3()) fPos = fStrawX-isign*sign*hitWireDistance[0];
                if (hitWireDistance[0]>fPar->GetTripletParameter3()) fPosNext = fStrawNextX+isign*sign*hitWireDistance[1];
              }
              paired = 1;
            }
            ...
            // Store doublets
            if (paired) {
              Double_t aslope = (fPos-fPosNext)/(fStrawZ-fStrawNextZ);
              ...
              Double_t s2aslope = s2sumwire/(fabs(fStrawZ-fStrawNextZ)*fabs(fStrawZ-fStrawNextZ));
...
Double_t ViewHitCollector::ChooseTheCombination(Straw** straw, Double_t* fRadius, Double_t* xGood, Double_t* slopeGood) {
  ...
  Int_t icounter(1);
  for (Int_t j=0; j<3; j++) z[j] = straw[j]->GetLocalPosition().Z();
  for (Int_t jc=0;jc<8;jc++){
    ...
    for (Int_t j=0;j<3;j++){
      x[j] = straw[j]->GetLocalPosition().X()+c[j]*fRadius[j];
