...
///
/// \Detailed
/// This class stores the parameters of the spectrometer geometry.
/// \EndDetailed

SpectrometerGeometry* SpectrometerGeometry::fInstance = nullptr;
...
SpectrometerGeometry * SpectrometerGeometry::GetInstance(){
  if (fInstance == nullptr) {
    fInstance = new SpectrometerGeometry();
  }
  return fInstance;
}
...
/// \MemberDescr
/// Creation of the geometry by defining the numbers and the positions of each
/// straw tube as created in NA62MC. Call CreateDataGeometry since the geometry
/// of data and MC is identical except for x-alignment.
/// \EndMemberDescr
  CreateDataGeometry();
}

void SpectrometerGeometry::CreateDataGeometry() {
  // cout_en(level,filter) << "[SpectrometerGeometry] Initialize variables" << std::endl;
    ...
    for (Int_t iv=0; iv < fPar->GetNViews(); iv++) {     // loop over views
      ...
      for (Int_t ip=0; ip < fPar->GetNPlanes(); ip++) {  // loop over planes
        ...
        for (Int_t is=0; is<fPar->GetNStraws(); is++) {  // loop over straws
          ...
          fStraws[strawid]->SetLocalPosition(TVector3(localx, localy, localz));
          fStraws[strawid]->SetPosition(TVector3(0,0,zPos));
          TVector3 strawAxis = TVector3(0,1,0);
          strawAxis.RotateZ(fViewRotationAngle[iv]);
          fStraws[strawid]->SetAxis(strawAxis);
        }
      }
    }
  }
  ...
  // cout_en(level, filter) << "[SpectrometerGeometry] Geometry created" << std::endl;
}
