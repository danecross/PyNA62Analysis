...
void SRBRawDecoder::ParseRawDecoderSettingsFile(TString RawDecFileName){
  ...
  while(Line.ReadLine(NA62ConditionsService::GetInstance()->Get(RawDecFileName))){
    if(Line.BeginsWith("#")) continue; //COMMENT LINE
      ...
      TObjArray * l = Line.Tokenize(" ");
      TString Line = static_cast<TObjString*>(l->At(1))->GetString();
      fSRBOffset = strtol(TString(Line(TRegexp("[0-9a-fA-F]+"))),NULL,16);
      delete l;
      continue;
      ...
      for(Int_t i = 0; i < NEntries; i++) {
        ...
        fSRBLatency[i] = strtol(TString(Line(TRegexp("[0-9a-fA-F]+"))),NULL,16);
      }
      delete l;
      continue;
    }
