...
CHODPrimMon::CHODPrimMon(TRootBrowser* mainwin, TString Name) : VPrimMon(mainwin, Name) {
  NBurst = 0 ;

  // set hist style
  fHPrimTime = new TH1F("fHPrimTimeCHOD","CHOD primitives rate                                        ",800, 0., 8000.);
  ...
  fHPrimTimeZoom1 = new TH1F("fHPrimTimeZoomCHOD","CHOD primitives rate                         ",80000, 0., 8000.);
  ...
  fHPrimDelta = new TH1F("fHPrimDeltaCHOD","dt of primitives (ns) ;Time Difference (ns);Counts",200000, -100000, 100000.);
  fHPrimDelta->GetXaxis()->SetTitle("ns");
  fHPrimDelta->GetYaxis()->SetTitle("");
  fHPrimDelta->SetLineColor(kRed);

  fHPrimDeltaTS = new TH1F("fHPrimDeltaTSCHOD","dt of primitives (FT units) ;Time Difference (x100 ps) ;Counts",2000, -1000, 1000.);
  fHPrimDeltaTS->GetXaxis()->SetTitle("FT units");
  fHPrimDeltaTS->GetYaxis()->SetTitle("");
  fHPrimDeltaTS->SetLineColor(kOrange);

  fHPrimID = new TH1F("fHPrimIDCHOD","CHOD primitive types",16, -0.5, 15.5);
  ...
  fHTimeBits = new TH1F("fHTimeBitsCHOD","Probability of the 40 tstamp bits (timestamp & finetime) to be high; TimeStamp & FineTime; Fraction", 40, -39.5,0.5) ;
  fHTimeBits->Sumw2() ;

  fHFineTime = new TH1F("fHFineTimeCHOD","Fine Time; FT; Counts", 256, -0.5,255.5) ;

  fHTimeStamp16   = new TH1F("fHTimeStamp16CHOD","Time Stamp below 4th bit;Time Stamp mod 16 ; Count", 16, -0.5,15.5) ;
  fHTimeStamp128  = new TH1F("fHTimeStamp128CHOD","Time Stamp below 7th bit;Time Stamp mod 128 ; Count", 128, -0.5,127.5) ;
  fHTimeStamp1024 = new TH1F("fHTimeStamp1024CHOD","Time Stamp below 10th bit;Time Stamp mod 1024 ; Count", 256, -0.5,1023.5) ;
  fHTimeStamp8192 = new TH1F("fHTimeStamp8192CHOD","Time Stamp below 13th bit;Time Stamp mod 8192 ; Count", 512, -0.5,8191.5) ;

  fHFineTimeStamp128   = new TH1F("fHFineTimeStamp128CHOD",
				 "Fine Time Stamp below 7th bit; Fine; Count", 128, -0.5,127.5) ;

  fHPrimFFT= new TH1D("fHPrimFFTCHOD","FFT - CHOD; Hz; ",65536,0.,10000.);
  fHPrimFFT->SetLineColor(kBlue);

  fHPrimTimeFFT = new TH1D("fHPrimTimeFFTCHOD","CHOD time profile for FFT; ns",65536,0.,6.5536E+9);
  ...
  TCanvas * cCHODPrimTS = AddCanvasTab("Rate");
  cCHODPrimTS->Divide(1,3);
  cCHODPrimTS->cd(1)->SetPad(.005, .500, .995, .995);
  cCHODPrimTS->cd(1);
  fHPrimTime->Draw();
  fPText->Draw();
  cCHODPrimTS->cd(2)->SetPad(.500, .005, .995, .495);
  cCHODPrimTS->cd(2);
  cCHODPrimTS->cd(2)->SetLogy(kTRUE);
  fHPrimDeltaTS->Draw();
  cCHODPrimTS->cd(3)->SetPad(.005, .005, .495, .495);
  cCHODPrimTS->cd(3);
  cCHODPrimTS->cd(3)->SetLogy(kTRUE);
  fHPrimDelta->Draw();

  TCanvas * cCHODZoom = AddCanvasTab("Burst structure");
  cCHODZoom->Divide(1,3);
  cCHODZoom->cd(1)->SetPad(.005, .500, .500, .995 );
  cCHODZoom->cd(1);
  fHPrimTimeZoom1->Draw();
  cCHODZoom->cd(2)->SetPad(.500, .500, .995, .995);
  cCHODZoom->cd(2);
  fHPrimFFT->Draw();
  cCHODZoom->cd(3)->SetPad(.005, .005, .995, .495);
  cCHODZoom->cd(3);
  fHPrimTimeZoom2->GetXaxis()->SetRangeUser(3000.,4000.);
  fHPrimTimeZoom2->Draw();

  TCanvas * cCHODPrimID = AddCanvasTab("Primitive IDs");
  cCHODPrimID->cd(1)->SetLogx(kTRUE);
  fHPrimID->Draw("hbar1");
  fHPrimID->SetFillColor(kGreen);
  fHPrimID->SetBarWidth(0.8);

  /*TCanvas * cCHODBits = */AddCanvasTab("Time Bits");
  fHTimeBits->GetYaxis()->SetRangeUser(0.4,0.6);
  fHTimeBits->Draw();

  TCanvas * cCHODTimeStamps = AddCanvasTab("Time Stamps");
  cCHODTimeStamps->Divide(2,2) ;
  cCHODTimeStamps->cd(1) ;
  fHTimeStamp16->Draw();
  cCHODTimeStamps->cd(2) ;
  fHTimeStamp128->Draw();
  cCHODTimeStamps->cd(3) ;
  fHTimeStamp1024->Draw();
  cCHODTimeStamps->cd(4) ;
  fHTimeStamp8192->Draw();

  /*TCanvas * cCHODFineTime = */AddCanvasTab("Fine Time");
  fHFineTime->Draw() ;

  /*TCanvas * cCHODFineTimeStamps = */AddCanvasTab("MultiPad Fine Time Stamps");
...
void CHODPrimMon::Update(TTree* tree, TString TimeRunBurst, TString time) {
  ...
  cout << "CHOD entries " << nEntries << endl;
  ...
  for (int iEnt = 0; iEnt < nEntries; iEnt++) {
    ...
    if(iEnt%100000==0){
      cout << "CHOD: " << (100.*iEnt)/nEntries << "% of processing complete \r";
    }
  ...
  fHPrimTime->SetTitle(Form("CHOD primitives rate - %s", TimeRunBurst.Data()));
  fHPrimTimeZoom1->SetTitle(Form("CHOD - %s", TimeRunBurst.Data()));
...
void CHODPrimMon::Rescale(TH1F* h){
