...
/// \class MUV2Reconstruction
/// \Brief
/// Main MUV2 reconstruction: starts from the analog signals of the photomultipliers (ADC Samples) stored in TMUV2Digi: the recontruction finds and fits the peaks creating a TRecoMUV2Hit for each of them.\n
/// The Hits are organized in vector (one per channel) ordered by time.\n
/// For each readout side ( top, bottom, jura and saleve ) the hits are placed in 1D clusters and then these clusters are used to build the 2D clusters corresponding to the final TRecoMUV2Candidate. \n
/// \EndBrief
/// \Detailed
/// The reconstruction makes use of the base class NA62VReconstruction, the class MUV2HitsCluster and of the classes in NA62MC/MUV2/Persistency (TRecoMUV2Event, TRecoMUV2Candidate, TRecoMUV2Hit).
///\n
/// The MUV2 reconstruction basic parameters are set in NA62Reconstruction/config/MUV2.conf
/// \n
/// =========
/// \n
/// MUV2.conf
...
/// =========
/// \n
/// Description of parameters can be found in MUV2.conf, here the parameters you may want to change to perform your own reconstruction:
/// \n\n
...
/// \n\n
/// SaveSaturated= 0 --> if 1 save in the MUV2Monitor/Histos folder a plot of the analog signals in case of ADC saturation
/// \n\n
/// SaveRefused= 0 --> if 1 save in the MUV2Monitor/Histos folder a plot of the analog signals in case 1 of the fitted peaks has been refused ( for reconstruction debugging )
/// \n
/// \n
///=====================================================================================================================================
/// \n
...
/// For each side of the detector the neighboring channels are clusterized in MUV2HitsCluster if the time distance is below the ChannelClusterTimeCut value defined in the MUV2.conf file.\n
/// The MUV2HitsCuster evaluate the basic characteristics of the cluster: Time, Position, Total collected charge ...
/// \n
/// \n
/// Candidates Construction \n
/// The 1D clusters coming from the different readout side are combined into TRecoMUV2Candidate (2D Cluster), respecting the quadrant structure of the detector, if the time difference is below the SideClusterTimeCut value defined in the MUV2.conf file.\n
...
Double_t   *fMUV2FitDataXptr=0, *fMUV2FitDataYptr=0;
Double_t   *fMUV2FitDataErrorXptr=0, *fMUV2FitDataErrorYptr=0;
Int_t  fNMUV2FitPoints=0;

MUV2Reconstruction::MUV2Reconstruction(TFile* HistoFile, TString ConfigFileName) :
  NA62VReconstruction(HistoFile, "MUV2", ConfigFileName),
  fCandidate(nullptr),
  fFitter(0),
  fGeomInstance(nullptr),
  fTPosDependency(0),fQPosDependency(0)
{

    fRecoEvent = new TRecoMUV2Event();
...
MUV2Reconstruction::~MUV2Reconstruction(){
...
void MUV2Reconstruction::ParseConfFile(TString ConfFileName) {

  //Default file for time alignment, detector calibration and baseline values
  fT0FileName="MUV2-T0.dat";
  TString QEqFileName="MUV2-QEqualization.dat";
  TString TimeDepFileName="MUV2-TimeDep.dat";

  fCalibrationParameter = 0.650;
  ...
  if(NA62ConditionsService::GetInstance()->Open(fT0FileName)==kSuccess){
    while (Line.ReadLine(NA62ConditionsService::GetInstance()->Get(fT0FileName))) {
      ...
      if (TMath::Abs(time) < 500.) fMUV2T0[(Channel-100)/50][Channel%50-1] = time;
  ...
  for (Int_t i=0; i<4; i++) {
    ...
    fMeanT0 += TMath::Mean(22,fMUV2T0[i]);
  }
  fMeanT0 /= 4.;
  ...
  while (Line.ReadLine(NA62ConditionsService::GetInstance()->Get(QEqFileName))) {
    if (Line.BeginsWith("#")) continue;
    		...
    		TObjArray * line = Line.Tokenize(" ");
    		fQPosDependency = new TF1 ("MUV2QPosDependency",static_cast<TObjString*>(line->At(1))->GetString().Data(),0,1500);
    		fQPosNParameters = fQPosDependency->GetNpar();
    		for (Int_t i=0; i<4; i++){
    		    for (Int_t j=0; j<22; j++) fMUV2QEq_pars[i][j] = new Double_t [fQPosNParameters];
    		  }
    		delete line;
    		continue;
    }
    ...
    fMUV2QEq[(Channel-100)/50][Channel%50-1] = 1./QEq;

    Int_t par_n = 0;

    while (par_n<fQPosNParameters && par_n+2<line->GetEntries()){
      fMUV2QEq_pars[(Channel-100)/50][Channel%50-1][par_n] = static_cast<TObjString*>(line->At(2+par_n))->GetString().Atof();
      par_n++;
    }
  ...
  while (Line.ReadLine(NA62ConditionsService::GetInstance()->Get(TimeDepFileName))) {
    if (Line.BeginsWith("#")) continue;
    		...
    		line = Line.Tokenize(" ");
    		fTPosDependency = new TF1 ("MUV2TPosDependency",static_cast<TObjString*>(line->At(1))->GetString().Data(),0,1500);
    		delete line;
    		continue;
    }
    line = Line.Tokenize(" ");
    ...
    Double_t par2 = static_cast<TObjString*>(line->At(3))->GetString().Atof();

    fMUV2TimeDep_par0[(Channel-100)/50][Channel%50-1] = par0;

    fMUV2TimeDep_par1[(Channel-100)/50][Channel%50-1] = par1;

    fMUV2TimeDep_par2[(Channel-100)/50][Channel%50-1] = par2;
...
void MUV2Reconstruction::Init(NA62VReconstruction* MainReco) {
  ...
  fGeomInstance = MUV2Geometry::GetInstance();
...
TDetectorVEvent * MUV2Reconstruction::Trigger(TDetectorVEvent * tEvent, Event* /*tGenEvent*/){
  return tEvent;
}

void MUV2Reconstruction::StartOfBurst(){
  NA62VReconstruction::StartOfBurst(); // common part for all the subdetectors

}

void MUV2Reconstruction::EndOfBurst(){
...
TRecoVEvent * MUV2Reconstruction::ProcessEvent(TDetectorVEvent* tEvent, Event* tGenEvent){
  ...
  for (Int_t  iHit=0; iHit<NDigis; iHit++){
    ...
    TMUV2Digi* Digi = static_cast<TMUV2Digi*>( Digis[iHit]);
    ...
    for (Int_t i=0; i<Nsamples; i++) {
      fMUV2FitDataX[i] = i;
      fMUV2FitDataErrorX[i] = 1./ClockPeriod;
      fMUV2FitDataY[i] = Amplitude[i];
      fMUV2FitDataErrorY[i] = 3.;
    ...
    for (UInt_t  iPeak=0; iPeak<PeakPosition.size(); iPeak++){
      ...
      if (PeakPosition.size()>1 && iPeak<(PeakPosition.size()-1) && (PeakPosition[iPeak+1] - PeakPosition[iPeak])< 8) {
        ...
        fMUV2FitDataXptr = &fMUV2FitDataX[(PeakPosition[iPeak]-7 >= 0 ? PeakPosition[iPeak]-7 : 0)];
        fMUV2FitDataYptr = &fMUV2FitDataY[(PeakPosition[iPeak]-7 >= 0 ? PeakPosition[iPeak]-7 : 0)];
        fMUV2FitDataErrorXptr = &fMUV2FitDataErrorX[(PeakPosition[iPeak]-7 >= 0 ? PeakPosition[iPeak]-7 : 0)];
        fMUV2FitDataErrorYptr = &fMUV2FitDataErrorY[(PeakPosition[iPeak]-7 >= 0 ? PeakPosition[iPeak]-7 : 0)];
        fNMUV2FitPoints = 1 + ((PeakPosition[iPeak+1]+3 < Nsamples ? PeakPosition[iPeak+1]+3 : Nsamples-1) - (PeakPosition[iPeak]-7 >= 0 ? PeakPosition[iPeak]-7 : 0));
        ...
        Limits[0] = 4; Limits[1] = 1.1; Limits[2] = 2.;
        fFitter->mnexcm("SET LIM",Limits,3,ierflg);
        ...
        Limits[0] = 7; Limits[1] = 1.1; Limits[2] = 2.;
        fFitter->mnexcm("SET LIM",Limits,3,ierflg);
        ...
        if (peak1>0. && charge1 < 1.e+7) {
          ...
          ThisHit = static_cast<TRecoMUV2Hit*>( fRecoEvent -> AddHit(Digi));
          ThisHit -> DecodeChannelID(Digi->GetChannelID());
          time1 -= GetT0Correction(Digi);

          if (Digi->GetQuadrant()>0){

            time1 -= fMUV2T0[Side][Channel];
            peak1 *= fMUV2QEq[Side][Channel];
            perr1 *= fMUV2QEq[Side][Channel];
            charge1 *= fMUV2QEq[Side][Channel];
            Qerr1 *= fMUV2QEq[Side][Channel];
            ...
            fHQVSChannel -> Fill(Digi->GetChannelID(),charge1/GetQPosAttenuation(Digi->GetChannelID(),30));
            if (TMath::Abs(time1 - NA62RecoManager::GetInstance()->GetEventHeader()->GetFineTime()*ClockPeriod/256.) < 20.) fHQVSChannel_L0intime -> Fill(Digi->GetChannelID(),charge1);
        ...
        if (peak2>0. && charge2 < 1.e+7) {
          ...
          if (Digi->GetQuadrant()>0){
            ...
            time2 -= fMUV2T0[Side][Channel];
            peak2 *= fMUV2QEq[Side][Channel];
            perr2 *= fMUV2QEq[Side][Channel];
            charge2 *= fMUV2QEq[Side][Channel];
            Qerr2 *= fMUV2QEq[Side][Channel];

            fHChannelsOccupancy -> Fill(Digi->GetChannelID());
            fHAmplitudeVSChannel -> Fill(Digi->GetChannelID(),peak2);
            fHSigmaVSChannel -> Fill(Digi->GetChannelID(),sigma2);
            fHQVSChannel -> Fill(Digi->GetChannelID(),charge2/GetQPosAttenuation(Digi->GetChannelID(),30));
            if (TMath::Abs(time2 - NA62RecoManager::GetInstance()->GetEventHeader()->GetFineTime()*ClockPeriod/256.) < 20.) fHQVSChannel_L0intime->Fill(Digi->GetChannelID(),charge2);
      ...
      //---------------------------------
        ...
        //Setting Fit data addresses for minuit
        fMUV2FitDataXptr = &fMUV2FitDataX[(PeakPosition[iPeak]-5 >= 0 ? PeakPosition[iPeak]-5 : 0)];
        fMUV2FitDataYptr = &fMUV2FitDataY[(PeakPosition[iPeak]-5 >= 0 ? PeakPosition[iPeak]-5 : 0)];
        fMUV2FitDataErrorXptr = &fMUV2FitDataErrorX[(PeakPosition[iPeak]-5 >= 0 ? PeakPosition[iPeak]-5 : 0)];
        fMUV2FitDataErrorYptr = &fMUV2FitDataErrorY[(PeakPosition[iPeak]-5 >= 0 ? PeakPosition[iPeak]-5 : 0)];
        fNMUV2FitPoints = 1 + ((PeakPosition[iPeak]+3 < Nsamples ? PeakPosition[iPeak]+3 : Nsamples-1) - (PeakPosition[iPeak]-5 >= 0 ? PeakPosition[iPeak]-5 : 0));
        ...
        if (peak_sigma<1.1 || peak_sigma>3. || peak_sigma/peak_amp<0) peak_sigma = 1.4;
        ...
        if (peak>0. && charge < 1.e+7) {
          ...
          if (Digi->GetQuadrant()>0){
            ...
            time -= fMUV2T0[Side][Channel];
            peak *= fMUV2QEq[Side][Channel];
            perr *= fMUV2QEq[Side][Channel];
            charge *= fMUV2QEq[Side][Channel];
            Qerr *= fMUV2QEq[Side][Channel];

            fHChannelsOccupancy -> Fill(Digi->GetChannelID());
            fHAmplitudeVSChannel -> Fill(Digi->GetChannelID(),peak);
            fHSigmaVSChannel -> Fill(Digi->GetChannelID(),sigma);
            fHQVSChannel -> Fill(Digi->GetChannelID(),charge/GetQPosAttenuation(Digi->GetChannelID(),30));
            if (TMath::Abs(time - NA62RecoManager::GetInstance()->GetEventHeader()->GetFineTime()*ClockPeriod/256.) < 20.) fHQVSChannel_L0intime->Fill(Digi->GetChannelID(),charge);
            fHHitTime -> Fill(fPars[2],Slot);
    ...
    if (Refused || Saturation || Underflow) {
      TGraph histo_temp (Nsamples, fMUV2FitDataX, fMUV2FitDataY);
      histo_temp.SetTitle(Form("Event_%d_Burst_%d_Channel_%d",fRecoEvent->GetID(), fRecoEvent->GetBurstID(),Digi->GetChannelID()));

      if (!(fHistoFile->GetDirectory(Form("MUV2Monitor/Histos/Burst_%d",fRecoEvent->GetBurstID()))))
        fHistoFile->mkdir(Form("MUV2Monitor/Histos/Burst_%d",fRecoEvent->GetBurstID()));

      fHistoFile->cd(Form("MUV2Monitor/Histos/Burst_%d",fRecoEvent->GetBurstID()));
  ...
  std::vector <TRecoMUV2Hit*> SideHits[4];
  ...
  for (Int_t i=0; i < HitsArray->GetEntries(); i++) {
    ...
    TRecoMUV2Hit *hit = static_cast<TRecoMUV2Hit*>(HitsArray->At(i));
    Double_t channelID = hit->GetChannelID();
    Int_t side = (channelID-100)/50;

    if (side < 0 || side > 3) {

      std::cerr << " MUV2 hit with wrong channelID " << channelID << std::endl;
  ...
  for (Int_t iSide=0; iSide < 4; iSide++) {
    ...
    while ((NHits[iSide] - NUsedHits[iSide]) > 0) {
      ...
      MUV2HitsCluster *cluster = new MUV2HitsCluster();
      ...
      const double TimeRange = SideHits[iSide][SeedIndex[iSide]]->GetTimeError();
  ...
  MUV2HitsCluster *HCluster, *VCluster;
  for (Int_t i=0; i<4; i+=2){

    for (UInt_t j=0; j<fSideCluster[i].size(); j++){
      HCluster = static_cast<MUV2HitsCluster*>( fSideCluster[i][j]);
      ...
      for (UInt_t k=0; k<fSideCluster[SecondSide].size(); k++){
        ...
        for (Int_t ihit=0; ihit < NHHits; ihit++) {
          ...
          TRecoMUV2Hit *Hhit = HHits[ihit];

          Int_t Channel = Hhit->GetChannelID();

          fQPosDependency->SetParameters(fMUV2QEq_pars[(Channel-100)/50][Channel%50-1]);

          HHits_charge[ihit] = Hhit->GetCharge() / fQPosDependency->Eval(TMath::Abs(VCluster->GetY()));

          fTPosDependency->SetParameters(fMUV2TimeDep_par0[(Channel-100)/50][Channel%50-1],fMUV2TimeDep_par1[(Channel-100)/50][Channel%50-1],fMUV2TimeDep_par2[(Channel-100)/50][Channel%50-1]);

          HHits_time[ihit] = Hhit->GetTime() + fTPosDependency->Eval(TMath::Abs(VCluster->GetY()));
        ...
        for (Int_t ihit=0; ihit < NVHits; ihit++) {
          ...
          TRecoMUV2Hit *Vhit = VHits[ihit];

          Int_t Channel = Vhit->GetChannelID();

          fQPosDependency->SetParameters(fMUV2QEq_pars[(Channel-100)/50][Channel%50-1]);

          VHits_charge[ihit] = Vhit->GetCharge() / fQPosDependency->Eval(TMath::Abs(HCluster->GetX()));

          fTPosDependency->SetParameters(fMUV2TimeDep_par0[(Channel-100)/50][Channel%50-1],fMUV2TimeDep_par1[(Channel-100)/50][Channel%50-1],fMUV2TimeDep_par2[(Channel-100)/50][Channel%50-1]);

          VHits_time[ihit] = Vhit->GetTime() + fTPosDependency->Eval(TMath::Abs(HCluster->GetX()));
        ...
        Int_t  Hindex = MUV2Geometry::GetInstance()->GetScintillatorAt(HCluster->GetX());
        Int_t  Vindex = MUV2Geometry::GetInstance()->GetScintillatorAt(VCluster->GetY());
        ...
        TRecoMUV2Candidate *ClusterCandidate = static_cast<TRecoMUV2Candidate*>( fRecoEvent->AddCandidate());
  ...
  for (Int_t i=0; i<Nhits; i++) {
    TRecoMUV2Hit *Hit1 = static_cast<TRecoMUV2Hit*>( HitsArray->At(i));
    if (Hit1->GetQuadrant()<=0) continue;
    if (Hit1->GetChannelID()%100>50) continue;
    for (Int_t j=0; j<Nhits; j++) {
      TRecoMUV2Hit *Hit2 = static_cast<TRecoMUV2Hit*>( HitsArray->At(j));

      if (Hit1->GetQuadrant()!=Hit2->GetQuadrant()) continue;
      if (Hit2->GetChannelID()%100 < 50) continue;
      if (TMath::Abs(Hit1->GetTime() - Hit2->GetTime())>10.) continue;
...
void MUV2Reconstruction::EndProcessing(){
  NA62VReconstruction::EndProcessing(); // call base class for raw hist output
  // Write histos
  SaveHistograms();
}

void MUV2Reconstruction::FillTimes(Double_t ReferenceTime) {
  //common part for all the subdetectors
  ...
  for (Int_t i=0; i<fRecoEvent->GetNHits(); i++) {
    ...
    TRecoMUV2Hit * Hit = static_cast<TRecoMUV2Hit*>( Hits->At(i));
    ...
    for (Int_t j=0; j<fRecoEvent->GetNHits(); j++){
      if (i==j) continue;
      TRecoMUV2Hit * Hit2 = static_cast<TRecoMUV2Hit*>( Hits->At(j));
    ...
    if (fHRecoHitTimeWrtReferenceVsROChannelNoT0) GetHRecoHitTimeWrtReferenceVsROChannelNoT0()->Fill(ROChannel,Hit->GetTime()-ReferenceTime+fMUV2T0[Side][Channel]+time_pos_corr);
  }
}

void MUV2Reconstruction::InitHistograms() {

  GetOrMakeDir(fHistoFile,"MUV2Monitor");
  GetOrMakeDir(fHistoFile,"MUV2Monitor/Histos");
  GetOrMakeDir(fHistoFile,"MUV2Monitor/Baselines");


  //Baselines
  fHistoFile->cd("MUV2Monitor/Baselines");

  AddHisto(fHDistDerBaseline = new TH1D ("DistDerBase","der",1000,-50,50));
  for (Int_t i=0; i<4; i++){
    for (Int_t j=0; j<22; j++){
      AddHisto(fHBaselines[i][j] = new TH1D (Form("Baseline_Channel_%d",101 + 50*i + j),Form("Baseline_Channel_%d",101 + 50*i + j),1000,0,1000));
    }
  }

  fHistoFile->cd("MUV2Monitor");
  ...
  AddHisto(fHChannelsHitMap = new TH2F("ChannelsHitMap", "Channels Hit Map",22,0,22,22,0,22));
  ...
  double hitMapBinEdges[221]= {0.}; hitMapBinEdges[0]=-129.8;
  for (int i=1; i<221; i++){ if(i<101||i>120) {hitMapBinEdges[i]=hitMapBinEdges[i-1]+1.19;} else {hitMapBinEdges[i]=hitMapBinEdges[i-1]+1.08;} }  hitMapBinEdges[110]=0.;
  AddHisto(fHHitMap = new TH2F("HitMap", "Calorimeter Front Face; Cluster x [cm]; Cluster y [cm]", 220, hitMapBinEdges, 220, hitMapBinEdges));

  AddHisto(fHClusterSeedChargeVsChannel = new TH2F("ClusterSeedChargeVsChannel", "Cluster Seed Charge vs Seed Channel; Channel ID; Charge [pC]", 200,100,300,1000,0,10000 ));
  ...
  AddHisto(fHSideClusterTimeSpreadVSChannelHit = new TH2F("SideClusterTimeSpreadVSChannelHit", "Time standard deviation (side cluster) vs amount of hit channels; #sigmaT [ns]; NHits", 80,0,16,22,0,22));
...
void MUV2Reconstruction::SaveHistograms() {
...
void MUV2Reconstruction::ResetHistograms() {
...
Double_t   MUV2Reconstruction::SinglePeakFunction(Double_t   x, Double_t   * pars){
...
void MUV2Reconstruction::ChiSquareSinglePeak(Int_t  &/*npar*/, Double_t   */*gin*/, Double_t   &f, Double_t   *par, Int_t  /*iflag*/){

  Double_t   chisquare = 0.;
  for (Int_t i=0; i<fNMUV2FitPoints; i++) {
    Double_t   delta = (SinglePeakFunction(fMUV2FitDataXptr[i],par) - fMUV2FitDataYptr[i]);
    Double_t   err2_x = SinglePeakFunction(fMUV2FitDataErrorXptr[i] + fMUV2FitDataXptr[i],par);
    err2_x -= SinglePeakFunction(fMUV2FitDataXptr[i] - fMUV2FitDataErrorXptr[i],par);
    err2_x *=  err2_x/4.;
    Double_t   err2_y = fMUV2FitDataErrorYptr[i]*fMUV2FitDataErrorYptr[i];
...
Double_t   MUV2Reconstruction::DoublePeakFunction(Double_t   x, Double_t   * pars){
...
void MUV2Reconstruction::ChiSquareDoublePeak(Int_t  &/*npar*/, Double_t   */*gin*/, Double_t   &f, Double_t   *par, Int_t  /*iflag*/){

  Double_t   chisquare = 0.;
  for (Int_t i=0; i<fNMUV2FitPoints; i++) {
    Double_t   delta = (DoublePeakFunction(fMUV2FitDataXptr[i],par) - fMUV2FitDataYptr[i]);
    Double_t   err2_x = DoublePeakFunction(fMUV2FitDataErrorXptr[i] + fMUV2FitDataXptr[i],par);
    err2_x -= DoublePeakFunction(fMUV2FitDataXptr[i] - fMUV2FitDataErrorXptr[i],par);
    err2_x *=  err2_x/4.;
    Double_t   err2_y = fMUV2FitDataErrorYptr[i]*fMUV2FitDataErrorYptr[i];
    Double_t   err2 = err2_x + err2_y;
    chisquare+=delta*delta / err2;
...
Double_t MUV2Reconstruction::GetTPosDelay (Int_t Channel, Double_t Position) {
  if (!fTPosDependency) return 0;
  fTPosDependency->SetParameters(fMUV2TimeDep_par0[(Channel-100)/50][Channel%50-1],fMUV2TimeDep_par1[(Channel-100)/50][Channel%50-1],fMUV2TimeDep_par2[(Channel-100)/50][Channel%50-1]);
  return fTPosDependency->Eval(Position);
}

Double_t MUV2Reconstruction::GetQPosAttenuation (Int_t Channel, Double_t Position) {
