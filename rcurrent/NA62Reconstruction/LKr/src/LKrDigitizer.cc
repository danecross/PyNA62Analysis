...
TDetectorVEvent * LKrDigitizer::ProcessEvent(TDetectorVEvent * tEvent){
  ...
  for(Int_t iLKrHit = 0; iLKrHit < NLKrHits; iLKrHit++){
    ...
    for (Int_t jsw=0; jsw<3; jsw++) {
      Double_t pedestal = jsw==0 ? fPar->GetPedRef(jx,jy) : fPar->GetCalOffset(jx,jy,jsw);
    ...
    if (maxgain>0){
      for(UInt_t iSample=0; iSample<NSamples; iSample++){
        ...
        Double_t currentSample = (PulseHeight-MinShapePulse)*current/pulseAmplitude;
    ...
    for (UInt_t iSample=0; iSample<NSamples; iSample++){
      ...
      Double_t ped = igain==0 ? fPar->GetPedRef(jx,jy) : fPar->GetCalOffset(jx,jy,igain); // Compute the pedestals cell by cell
      Double_t currentSample = (PulseHeight-MinShapePulse)*(current*fadc)/pulseAmplitude; // Compute the ADC counts
      ...
      Double_t count = currentSample+noiseUncorr+noiseCorr+ped; // Add noise and pedestals to the ADC counts
