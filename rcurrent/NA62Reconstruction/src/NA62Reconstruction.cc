...
NA62Reconstruction::NA62Reconstruction(TObjArray * InputFileNameList, TString ConfFileName, TFile * OutputFile, Int_t NEvt, Int_t NEvtPerFile, Int_t JumpNEvt, UInt_t Seed) :
  NA62VReconstruction(OutputFile , "NA62",ConfFileName), fInputFileNameList(InputFileNameList), fBinaryEvent(0,0), fNEvt(NEvt), fNEvtPerFile(NEvtPerFile), fJumpNEvt(JumpNEvt), fGlobalSeed(Seed), fTimer(true){
...
NA62Reconstruction::NA62Reconstruction(TString InputListFileName, TString ConfFileName, TString OutputFileName, Int_t NEvt, Int_t NEvtPerFile, Int_t JumpNEvt, UInt_t Seed, Int_t NFiles) :
  NA62VReconstruction(TFile::Open(OutputFileName.Data(), "RECREATE"), "NA62",ConfFileName), fInputListFileName(InputListFileName), fBinaryEvent(0,0), fNEvt(NEvt), fNEvtPerFile(NEvtPerFile), fJumpNEvt(JumpNEvt), fGlobalSeed(Seed), fTimer(true) {
    /// \MemberDescr
...
void NA62Reconstruction::Init(){
  ...
  // NA62ConditionsService: set current run and burst ID
  Int_t ConditionsServiceRunID   = 6610; //default
  Int_t ConditionsServiceBurstID =    0; //default
  TObjArray *subStrL;
  if(fIsRawData) subStrL = TPRegexp("^.*-(\\d{6})-(\\d{4})(.calib)?.dat$").MatchS(static_cast<TObjString*>(fInputFileNameList->At(0))->GetString());
  else           subStrL = TPRegexp("^.*_dr(\\d{4,6})_r(\\d{6,8}).root$").MatchS(static_cast<TObjString*>(fInputFileNameList->At(0))->GetString());
  if(subStrL->GetLast()>=2){
    ...
    if(fIsRawData) ConditionsServiceBurstID = static_cast<TObjString *>(subStrL->At(2))->GetString().Atoi(); //ignore it for MC
  }
  else { //error: unknown format of the input file
    std::cerr << "[NA62Reconstruction] WARNING: file name '" << static_cast<TObjString*>(fInputFileNameList->At(0))->GetString() << "' has an unknown format.. Failed to extract current Run and Burst IDs!" << std::endl;
    std::cerr << "[NA62Reconstruction]          --> ConditionsService will use the default run-burst IDs: " << Form("%06d-%04d",ConditionsServiceRunID,ConditionsServiceBurstID) << std::endl;
  }
  ...
  if(!fIsRawData){
    ...
    fBurstTime = GetRunTimeFromDB(ConditionsServiceRunID);
    std::cout << "[NA62Reconstruction] Run time found for the run " << ConditionsServiceRunID << " : Time = " << fBurstTime << std::endl;
  }
  else {
  ...
  if(fOutputStage == kStageDigis || fOutputStage == kStageReco){
    ...
    fOutputBranchEvents.push_back(new EventVector);
    if(fOutputStage == kStageReco && fWriteDigis) {
  ...
  for(Int_t iReco = 0; iReco < fNInputBranches; iReco++){
    ...
    if(!fIsRawData && !FindMCChain("MC"))
      ...
      if(ProcessingSupported){
        ...
        fInputEvents.push_back(0);
        fEvents.push_back(0);
        fRecoEvents.push_back(0);
        ...
        if(fOutputStage == kStageDigis && DigiEvent){
          ...
          if(FindOutputBranchEvent("SpecialTrigger")){
            ...
            else           FindOutputBranchEvent("SpecialTrigger")->push_back(0);
          }
        ...
        else if(fOutputStage == kStageReco && Reco->GetRecoEvent()){
          if(FindOutputBranchEvent(fOutputStageName.Data())) FindOutputBranchEvent(fOutputStageName.Data())->push_back(Reco->GetRecoEvent());
          if(FindOutputBranchEvent("SpecialTrigger")){
            if(RawDecoder) FindOutputBranchEvent("SpecialTrigger")->push_back(RawDecoder->GetSpecialTriggerEvent());
            else           FindOutputBranchEvent("SpecialTrigger")->push_back(0);
          }
          if(fWriteDigis && FindOutputBranchEvent("Digis")) FindOutputBranchEvent("Digis")->push_back(Reco->GetRecoEvent()->GetDigiEvent());
  ...
  if((fOutputStage == kStageDigis || fOutputStage == kStageReco) && FindOutputTree(fOutputStageName)){
...
void NA62Reconstruction::ParseConfigFile(TString ConfFileName) {
  ...
  while(Line.ReadLine(confFile)) {
    ...
    else if (Line.BeginsWith("WriteDigis=")) {
      fWriteDigis = TString(Line(TRegexp("[01]"))).Atoi();
    }
...
Bool_t NA62Reconstruction::DecodeEvent(UInt_t * &pDataBuffer, std::vector<UInt_t> &DataBuffer) {
  ...
  l1Struct HLTparameters;
  HLTparameters.l1Mask[0].ktag.configParams.l1TrigProcessID = 1;
  HLTparameters.l1Mask[0].ktag.configParams.l1TrigMaskID = 0; //0 for CHOD, 1 for RICH, 2 for KTAG, 3 for LAV, 4 for MUV3, 5 for Straw
  HLTparameters.l1Mask[0].ktag.configParams.l1TrigEnable = 1;
  HLTparameters.l1Mask[0].ktag.configParams.l1TrigLogic = 1;
  HLTparameters.l1Mask[0].ktag.configParams.l1TrigFlag = 0;
  HLTparameters.l1Mask[0].ktag.configParams.l1TrigRefTimeSourceID = 0;
  HLTparameters.l1Mask[0].ktag.configParams.l1TrigOnlineTimeWindow = 5;
  na62::KtagAlgo::initialize(0, HLTparameters.l1Mask[0].ktag);

  HLTparameters.l1Mask[0].lav.configParams.l1TrigProcessID = 2;
  HLTparameters.l1Mask[0].lav.configParams.l1TrigMaskID = 0;
  HLTparameters.l1Mask[0].lav.configParams.l1TrigEnable = 1;
  HLTparameters.l1Mask[0].lav.configParams.l1TrigLogic = 1;
  HLTparameters.l1Mask[0].lav.configParams.l1TrigFlag = 0;
  HLTparameters.l1Mask[0].lav.configParams.l1TrigRefTimeSourceID = 0;
  HLTparameters.l1Mask[0].lav.configParams.l1TrigOnlineTimeWindow = 5;
  na62::LAVAlgo::initialize(0, HLTparameters.l1Mask[0].lav);

  HLTparameters.l1Mask[0].straw.configParams.l1TrigProcessID = 3;
  HLTparameters.l1Mask[0].straw.configParams.l1TrigMaskID = 0;
  HLTparameters.l1Mask[0].straw.configParams.l1TrigEnable = 1;
  HLTparameters.l1Mask[0].straw.configParams.l1TrigLogic = 1;
  HLTparameters.l1Mask[0].straw.configParams.l1TrigFlag = 0;
  HLTparameters.l1Mask[0].straw.configParams.l1TrigRefTimeSourceID = 0;
  HLTparameters.l1Mask[0].straw.configParams.l1TrigOnlineTimeWindow = 5;
  na62::StrawAlgo::initialize(0, HLTparameters.l1Mask[0].straw);

  int const max_fragment_number = 32;
  na62::l0::Subevent subevent(max_fragment_number);
      ...
      if (not ((EventHeader->GetTriggerType() & 0xff) == 0x08) and // TRIGGER_L2_TIMEOUT
        ...
        if (fRecoRawTable[iRaw] == "Cedar" or fRecoRawTable[iRaw] == "LAV" or fRecoRawTable[iRaw] == "Spectrometer") {
          ...
          if (fRecoRawTable[iRaw] == "Cedar") {
            cedar_trigger = na62::KtagAlgo::processKtagTrigger(0, decoded_event, &l1Info);
            ...
            lav_trigger = na62::LAVAlgo::processLAVTrigger(0, decoded_event, &l1Info);
            ...
            // std::cout <<"LAV trigger word mask 0: "<< l1Info.getL1LAVTrgWrd(0) << " and lav trigger" << lav_trigger << std::endl;

          } else if (fRecoRawTable[iRaw] == "Spectrometer") {
            EventHeader->GetHLTEvent()->Clear();
            na62::StrawAlgo runstraw;
            straw_trigger = runstraw.processStrawTrigger(0, decoded_event, &l1Info);
...
Bool_t NA62Reconstruction::NextEvent(){
  ...
  if (fIsRawData){
    ...
    FindMCChain("MC")->GetEntry(fNProcessedEventsInTotal);
...
TRecoVEvent * NA62Reconstruction::ProcessEvent(TDetectorVEvent*, Event*){
  ...
  while(TriggerStatus){
    if(!fIsRawData || (fOutputStage == kStageReco || fOutputStage == kStageHistos || fOutputStage == kStageBinary)){
    ...
    if(TriggerStatus){
      ...
      for(Int_t iReco = 0; iReco < fNReconstructions; iReco++){
        if((fOutputStatus==kStatusGood || fOutputStatus>fSkipEventsLevel) && (fOutputStage == kStageReco || fOutputStage == kStageHistos)){
        ...
        if ((fOutputStatus==kStatusGood || fOutputStatus>fSkipEventsLevel) && (fOutputStage == kStageDigis || fOutputStage == kStageReco) && !fContinuousReading){
          if(fInputEvents[iReco]->IsA()->InheritsFrom("TSpecialTriggerEvent")){
            ...
            if(fOutputStage == kStageReco && fWriteDigis && FindOutputTree("Digis")) {
  ...
  for(Int_t iReco = 0; iReco < fNReconstructions; iReco++){
    if(fOutputStage == kStageDigis || fOutputStage == kStageReco){
      for(UInt_t iTree=0;iTree<fOutputTrees.size();iTree++){
        ...
        else if(fOutputStage == kStageReco && fWriteDigis) {
...
#ifdef ONLINEHLT
  na62::LAVAlgo::loadConfigurationFile(NA62ConditionsService::GetInstance()->GetFullPath("LAV-RawDecoderSettings.dat").Data());
  na62::StrawAlgo::loadConfigurationFile(NA62ConditionsService::GetInstance()->GetFullPath("Spectrometer-RawDecoderSettings.dat").Data(),
  ...
  for(Int_t iReco = 0; iReco < fNReconstructions; iReco++){
    if(fFillTimesEnabled) {
      ...
      if(fT0ReferenceDetectors[iReco]=="Trigger"){
        ...
        if((fOutputStatus==kStatusGood || fOutputStatus>fSkipEventsLevel) && (fOutputStage == kStageReco || fOutputStage == kStageHistos) && !fEvents[iReco]->IsA()->InheritsFrom("TSpecialTriggerEvent"))
...
void NA62Reconstruction::DecodeDIMBlock(UInt_t *pDataBuffer, UInt_t * NextOffset){
  ...
  while(pDataBuffer < NextOffset){ // LOOP ON ALL SUBDETECTOR BLOCKS
    ...
    if(/*0<=SubDetID && True by definition as SubDetID is unsigned*/ SubDetID<MAXNDETECTORS) {
      ...
      StoreDIMBlock(fRecoRawTable[SubDetID],pDataBuffer,NumberOfWords,SpecTrigEvent);
...
void NA62Reconstruction::StoreDIMBlock(const TString name, UInt_t * pDataBuffer, UInt_t NumberOfWords, TSpecialTriggerEvent *SpecTrigEvent){
  if(name=="DIM") {
...
Long_t NA62Reconstruction::GetRunTimeFromDB(UInt_t RunNumber) {
  Long_t RunTime = -1;
