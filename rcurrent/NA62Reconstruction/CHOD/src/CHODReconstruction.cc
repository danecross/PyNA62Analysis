...
///    TRecoCHODCandidate* fCandidate = static_cast<TRecoCHODCandidate*>(CHODEvent->GetCandidate(0));
///    Double_t x_chod = fCandidate->GetHitPosition().X();
///    Double_t y_chod = fCandidate->GetHitPosition().Y();
///    Int_t *hit_indexes = fCandidate->GetHitsIndexes();
///    // get first hit
...
TRecoVEvent * CHODReconstruction::ProcessEvent(TDetectorVEvent* tEvent, Event* tGenEvent){
  ...
  if(TimeCandidateNHitPairs>0){
    ...
    fCandidate = static_cast<TRecoCHODCandidate*>( fRecoEvent->AddCandidate());
    // add two hits to the TimeCandidate
    fCandidate->AddHit(TimeCandidateHitIndexV);
    fCandidate->AddHit(TimeCandidateHitIndexH);

    TVector2 CenterOfMassPosition = TVector2 (TimeCandidateCoordinateX,TimeCandidateCoordinateY);
    fCandidate->SetTime(TimeCandidateTime);
    fCandidate->SetHitPosition(CenterOfMassPosition);
    fCandidate->SetNHitPairs(TimeCandidateNHitPairs);
...
void CHODReconstruction::QuadrantHitAssociation(Int_t iRecoHit,Int_t jRecoHit) {
  ...
  fCandidate = static_cast<TRecoCHODCandidate*>( fRecoEvent->AddCandidate());
  fCandidate->AddHit(iRecoHit);
  fCandidate->AddHit(jRecoHit);
  Double_t AverageTime = 0.5*(TimeVCorr+TimeHCorr);
  TVector2 CandidatePosition = TVector2(HitPositionV.X(),HitPositionH.Y());
  fCandidate->SetTime(AverageTime);
  fCandidate->SetHitPosition(CandidatePosition);
  fCandidate->SetNHitPairs(1);
