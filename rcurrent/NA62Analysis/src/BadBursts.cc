...
Bool_t BadBursts::IsBad(UInt_t BurstID) {
  ...
  UInt_t BadBurstMask = 0;
  if (posBurstID<fBadBurstIDList.size()) { // burst found in the BadBurstID list
    BadBurstMask = fBadBurstIDList_mask[posBurstID];
  }

  // Debug
  //cout << "BadBurstCheck:    "; for (Int_t i=0; i<32; i++) cout << ((fSystemsActive>>i)&1); cout << endl;
  //cout << "ThisBurst(in CDB):"; for (Int_t i=0; i<32; i++) cout << ((BadBurstMask>>i)&1);   cout << endl;
