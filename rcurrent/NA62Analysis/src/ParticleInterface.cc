...
const NA62Particle ParticleInterface::FindParticle(int pdgID) const{
	/// \MemberDescr
	/// \param pdgID : pdg code of the particle
	/// \return G4ParticleDefinition corresponding to the provided particle pdg code
	/// \EndMemberDescr

	return NA62Particle(fTable->GetParticle(pdgID));
}

const NA62Particle ParticleInterface::FindParticle(TString name) const{
	/// \MemberDescr
	/// \param name : name of the particle
	/// \return G4ParticleDefinition corresponding to the provided particle name
	/// \EndMemberDescr

	return NA62Particle(fTable->GetParticle(name.Data()));
}
