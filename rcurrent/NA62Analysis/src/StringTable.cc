...
void StringTable::AddColumn(TString id, TString title){
	...
	fOrder.insert(std::pair<const TString, int>(id, fColumns));
...
void StringTable::AddColumn(int nbr){
	...
	while(nbr>0){
		fOrder.insert(std::pair<const TString, int>(fColumns, fColumns));
		fContent.push_back(std::vector<TString>());
		fColumns++;
		fRows++;
		nbr--;
...
void StringTable::Print(TString prefix, std::ostream &s) const{
	/// \MemberDescr
	/// \param prefix : prefix to add in front of each line
