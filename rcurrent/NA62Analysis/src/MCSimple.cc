...
void MCSimple::GetRealInfos( Event* MCTruthEvent){
	...
	for (Int_t i=0; i < MCTruthEvent->GetNKineParts(); i++)	{
		KinePart* kinePart = static_cast<KinePart*>(MCTruthEvent->GetKineParts()->At(i));
		std::cout << trace() << "Found in MCTruth : (ID: " << kinePart->GetID() << ", ParentID: " << kinePart->GetParentID() << ", PDGCode: " << kinePart->GetPDGcode() << ")" << std::endl;
		...
		for(itPTree=tempParticleTree.begin(); itPTree!=tempParticleTree.end(); ++itPTree){
			...
			if(pTreeNode!=nullptr){
				...
				std::cout << trace() << "Found parent tree ... Adding particle" << std::endl;
		...
		if(pTreeNode==nullptr){
			...
			std::cout << trace() << "Parent tree not found ... Creating new tree" << std::endl;
			tempParticleTree.push_back(new ParticleTree(kinePart));
		}

		//Test signature
		std::cout << trace() << "Signature : " << kinePart->GetParentID() << "," <<  kinePart->GetPDGcode() << std::endl;
		std::cout << trace() << "Available signatures :" << std::endl;
		if(TestLevel(Verbosity::kTrace)){
			for(it = testStruct.begin(); it!=testStruct.end(); ++it){
				std::cout << trace() << it->first.first << "," << it->first.second << "," << it->second << std::endl;
			}
		}
		if((it = testStruct.find(std::pair<int,int>(kinePart->GetParentID(), kinePart->GetPDGcode())))!=testStruct.end()){
			...
			std::cout << trace() << "Signature found" << std::endl;
	...
	std::cout << trace() << "Merging trees ... " << std::endl;
	for(ritPTree=tempParticleTree.rbegin(); (ritPTree!=tempParticleTree.rend()) && (tempParticleTree.size()>1);){
		for(itPTree=tempParticleTree.begin(); itPTree!=tempParticleTree.end(); ++itPTree){
			...
			if(pTreeNode!=nullptr){
				...
				std::cout << trace() << "Parent tree found ... Merging" << std::endl;
				...
				std::cout << trace() << "Unable to build decay tree. Particle with ID " << (*ritPTree)->GetParentID() << std::endl;
				++ritPTree;
			}
		}
	}
	...
	if(testStruct.size()==0) fStatus = kComplete;
		...
		fStatus = kMissing;
		for(it=testStruct.begin(); it!=testStruct.end(); ++it){
			std::cout << debug() << "Missing particle " << (*it).first.second << " with parent " << (*it).first.first << std::endl;
		}
	}
...
int MCSimple::AddParticle(int parent, int type){
	...
	std::cout << extended() << "Request for MC particle with ID " << type << " and parent " << parent << std::endl;
	fStruct.insert(std::pair<const std::pair<int,int>, int>(std::pair<int,int>(-parent,type), fStruct.size()+1));
	return fStruct.size();
}
...
void MCSimple::ReplaceID(std::multimap<std::pair<int,int>,int> &s, int seqID, int particleID){
	...
	for(it=s.begin(); it != s.end() && it->first.first<0;){
		if((*it).first.first == -seqID){
			s.insert(std::pair<const std::pair<int,int>, int>(std::pair<int,int>(particleID,it->first.second), it->second));
			it2 = it;
			if(it!=s.begin()){
				++it;
				s.erase(it2);
