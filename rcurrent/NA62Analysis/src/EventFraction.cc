...
void EventFraction::AddCounter(TString name, int * const v){
	...
	fCounters.insert(std::pair<const TString,int*>(name, v));
...
void EventFraction::PrintToStream(std::ostream &s) const{
	/// \MemberDescr
	...
	TH1F g("g", "", fCounters.size(), 0, fCounters.size());
	TH1F t("t", "", fCounters.size(), 0, fCounters.size());
	...
	for(it=fSequence.begin(); it!=fSequence.end(); ++it){
		if(isRelative){
			for (int ic = 0; ic < *(fCounters.find(*it)->second); ic++)
                                g.Fill(i);
                        for (int ic = 0; ic < *(fCounters.find(fSampleSizeCounter)->second); ic++)
                                t.Fill(i);
		}
		i++;
	}
	...
	if(isRelative){
		as.Divide(&g, &t); // default error interval: Clopper-Pearson
	}
...
void EventFraction::WriteTable(TString filePrefix) const{
	...
	fd.open(fileName.Data(), std::ofstream::out);
	PrintToStream(fd);
	fd.close();
}
