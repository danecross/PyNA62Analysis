...
void CounterHandler::NewEventFraction(TString name){
	...
	fEventFraction.insert(std::pair<const TString,EventFraction*>(name, new EventFraction(name)));
}
void CounterHandler::AddCounterToEventFraction(TString efName, TString cName){
	...
	if((ptr=fEventFraction.find(efName))!=fEventFraction.end()){
		...
		else if(NA62Analysis::Configuration::ConfigSettings::global::fNonExistingWarning)
			std::cerr << "Counter " << cName << " doesn't exist." << std::endl;
	}
	else if(NA62Analysis::Configuration::ConfigSettings::global::fNonExistingWarning)
		std::cerr << "EventFraction table " << efName << " doesn't exist." << std::endl;
}
...
void CounterHandler::BookCounter(TString name){
	...
	fCounters.insert(std::pair<const TString,int>(name, 0));
}
...
void CounterHandler::IncrementCounter(TString name, int v){
	...
	if((ptr=fCounters.find(name))!=fCounters.end()){
		...
		std::cerr << "Counter " << name << " doesn't exist." << std::endl;
}
