...
void VertexCDA::Process(int){
	...
	if(GTKEvent->GetNCandidates()==1){
		KaonPosition = static_cast<TRecoGigaTrackerCandidate*>(GTKEvent->GetCandidate(0))->GetPosition(2);
		KaonPosition.SetZ(KaonPosition.Z()+90932.5);
		KaonMomentum = static_cast<TRecoGigaTrackerCandidate*>(GTKEvent->GetCandidate(0))->GetMomentum();
	...
	if(SpectrometerEvent->GetNCandidates()==1){
		PipPosition = static_cast<TRecoSpectrometerCandidate*>(SpectrometerEvent->GetCandidate(0))->GetPositionBeforeMagnet();
		PipMomentum.SetXYZ(static_cast<TRecoSpectrometerCandidate*>(SpectrometerEvent->GetCandidate(0))->GetSlopeXBeforeMagnet(),
				static_cast<TRecoSpectrometerCandidate*>(SpectrometerEvent->GetCandidate(0))->GetSlopeYBeforeMagnet(),
				1);
		PipMomentum.SetMag(static_cast<TRecoSpectrometerCandidate*>(SpectrometerEvent->GetCandidate(0))->GetMomentum());
