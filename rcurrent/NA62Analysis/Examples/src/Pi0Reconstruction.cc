...
void Pi0Reconstruction::Process(int){
	...
	if(fMCSimple.fStatus == MCSimple::kMissing){
		...
		for(int iPart=0; iPart<MCTruthEvent->GetNKineParts(); iPart++){
			FillHisto("pdgID", static_cast<KinePart*>(MCTruthEvent->GetKineParts()->At(iPart))->GetParticleName(), 1);
		}
		withMC = false;
	}
	...
	for(int iLKr=0; iLKr<LKrEvent->GetNCandidates(); iLKr++){
		TRecoLKrCandidate *lkrCand = static_cast<TRecoLKrCandidate*>(LKrEvent->GetCandidate(iLKr));
		KinePart *part = new KinePart();
		...
		photonOrder.insert(pair<const double, int>(lkrCand->GetClusterEnergy()*1000*calibMult + calibConst, photons.size()));
	...
	if(photonOrder.size()>=2){
		...
		for(photonIt = photonOrder.rbegin(); photonIt != photonOrder.rend(); ++photonIt){
			...
			for(++photonIt2; photonIt2 != photonOrder.rend(); ++photonIt2){
				...
				double pi0Mass = sqrt(pow(g1->GetInitialEnergy() + g2->GetInitialEnergy(),2) - (g1->GetInitialMomentum() + g2->GetInitialMomentum()).Mag2());
				//insert the invariant mass divided by the pi0 mass in the map
				cID.insert(pair<const double, int>(fabs((pi0Mass/134.9766)-1), i));
				++i;
			}
		}
