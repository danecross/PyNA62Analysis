...
void LKrPhotonMC::Process(int iEvent){
	...
	TRecoLKrEvent *lkr = GetEvent<TRecoLKrEvent>();
	...
	OutputState state;
	vertex = *GetOutput<TVector3>("VertexCDA.Vertex", state);
	...
	for(int i=0; i<lkr->GetNCandidates(); i++){
		TRecoLKrCandidate *lkrCand = static_cast<TRecoLKrCandidate*>(lkr->GetCandidate(i));
		...
		KinePart *photonCandidate = CreateStandardCandidate("LKrCandidates");
	...
	while(tempVDouble.size()>0){
		int vId = getMostEnergetic(tempVDouble);
		...
		TRecoLKrCandidateMC *lkrCandMC = static_cast<TRecoLKrCandidateMC*>(fCandidates->ConstructedAt(candidatesNumber));
	...
	for(unsigned int i=0; i<clusterLKr.size(); i++){
		double d = distance3D(gamma1, clusterLKr[i]);
		if(d<distance1){
			d_sel1 = i;
			distance1 = d;
		}
	...
	if(d_sel1!=d_sel2){
		if(candidatesNumber>=1 && d_sel1!=-1 && distance1<thresholdgamma){
			...
			static_cast<TRecoLKrCandidateMC*>(fCandidates->At(d_sel1))->SetMCParticleID(fMCSimple["gamma"][0]->GetID());
		}
		if(candidatesNumber>=2 && d_sel2!=-1 && distance2<thresholdgamma){
			//Setting correct association
			static_cast<TRecoLKrCandidateMC*>(fCandidates->At(d_sel2))->SetMCParticleID(fMCSimple["gamma"][1]->GetID());
		}
	}
