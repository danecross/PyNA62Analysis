...
VertexLSF::VertexLSF(Int_t ntrk) :
  fChi2(0.), fNtrks(0), fZref(180000.), fVertex(0,0,0) {
  /// \MemberDescr
  /// Vertex constructor
...
void VertexLSF::AddTrack(TRecoSpectrometerCandidate* cand) {
  /// \MemberDescr
  /// Add a spectrometer track candidate to the fit.
  fCharge.push_back(cand->GetCharge());
  fGTKtrk.push_back(0);
  TMatrixD cov(5,5), par(5,1);
  TMatrixD C(5,5);
  for (Int_t m=0; m<5; m++) {
    for (Int_t n=0; n<5; n++) {
...
void VertexLSF::AddGTKTrack(TMatrixD par, TMatrixD cov, Double_t zpos) {
  ...
  TMatrixD &Par = *(TMatrixD*)fT[fNtrks];
  Par = par;
  PropagateGTKTrack(fNtrks, zpos, fZref);
  fNtrks++;
}
