...
CedarMCTester::CedarMCTester(Core::BaseAnalysis *ba) :
		Analyzer(ba, "CedarMCTester"),
		fhAllPositions(nullptr),
		fhKTAGPositions(nullptr)
{
...
void CedarMCTester::InitHist() {
    ...
    //RZ projections
    TH2F * fhRZPositions = new TH2F( "RZEndPoints", "All particle end points",
    ...
    //pmt rates
    TH1F * hrate = new TH1F ( "Rate", "PMT rate" , 80, 9.5, 89.5 );
    ...
    //Photon counts
    TH1F * hcount = new TH1F ( "NPhotons", "Number of photons",
            100, -0.5, 99.5 );

    hcount->GetXaxis()->SetTitle( "Number of photons" );
    hcount->GetYaxis()->SetTitle( "Number of events" );

    TH1F * htotalcount = new TH1F( "NPhotonsTotal", "Total number of photons" ,
            500, -0.5, 499.5  );
    ...
    BookHisto( "PhotonsNPhotonsTotal",
            static_cast<TH1F*>( htotalcount->Clone( "NPhotonsTotal") ),
            refresh, "Photons/Standard");
    ...
    BookHistoArray( "PhotonsNPhotons",
            static_cast<TH1F*>(  hcount->Clone("NPhotons") ),
            8, refresh, "Photons/Standard" );
    ...
    //ZRProfile
    TH2F * hZRProfile = new TH2F( "ZRProfile" , "Z  and r coordinate of all hits" ,
    ...
    BookHisto( "PhotonsZRProfile",
            static_cast<TH2F*>( hZRProfile->Clone("ZRProfile") ),
    ...
    //Diaphragm
    TH2F * hDiaphragm = new TH2F( "Diaphragm",
    ...
    BookHisto( "PhotonsDiaphragm",
            static_cast<TH2F*>(  hDiaphragm ->Clone( "Diaphragm" ) ),
            refresh, "Photons/RayTracing" );
    ...
    //Window
    TH2F * hWindow = new TH2F( "Window", " Photon positions at quartz window",
            100, -25, 25, 100, -25, 25);
    hWindow->GetXaxis()->SetTitle( "X (mm)" );
    hWindow->GetYaxis()->SetTitle( "Y (mm)" );

    BookHistoArray( "PhotonsWindow",
            static_cast<TH2F*>( hWindow ->Clone("Window" )),
    ...
    //Lightguide
    TH2F * hLightGuide = new TH2F("LightGuide",
...
void CedarMCTester::Process(int){
    ...
    TCedarEvent *CedarEvent = GetEvent<TCedarEvent>();
    ...
    for (int iHit = 0 ; iHit != nHits ; ++ iHit) {
        ...
        //Hit in diaphragm
            ...
            double WaveLength = hc / hit->GetEnergy();
            FillHisto("PhotonsDiaphragm", radius, WaveLength);
            if (photoelectron) FillHisto("PhotoelectronsDiaphragm", radius, WaveLength);
        }
    }
...
void CedarMCTester::EndOfJobUser() {
