...
DownstreamTrackBuilder::DownstreamTrackBuilder(Core::BaseAnalysis *ba) :
  ...
  fCDAcomp2->SetLine1Point1(0.0, 0.0, fZTrim5); // a point on the nominal beam axis
  fCDAcomp2->SetDir1(BeamParameters::GetInstance()->GetNominalBeamThreeMomentum());
}
...
void DownstreamTrackBuilder::StartOfRunUser() {
  ...
  fCDAcomp1->SetLine1Point1(BeamParameters::GetInstance()->GetBeamXYZ());
  fCDAcomp1->SetDir1(BeamParameters::GetInstance()->GetBeamThreeMomentum());
}
...
void DownstreamTrackBuilder::Process(Int_t) {
  ...
  for (Int_t iTrack=0; iTrack<STRAWevent->GetNCandidates(); iTrack++) {
    ...
    fTrack.SetTrackTime           (Scand->GetLeadingTime()); // 1.2ns resolution, as opposed to 5ns for GetTime()
    ...
    fCDAcomp1->SetLine2Point1(Scand->GetPositionBeforeMagnet());
    fCDAcomp1->SetDir2(Scand->GetThreeMomentumBeforeMagnet());
    fCDAcomp1->ComputeVertexCDA();
    fCDAcomp2->SetLine2Point1(Scand->GetPositionBeforeMagnet());
    fCDAcomp2->SetDir2(Scand->GetThreeMomentumBeforeMagnet());
