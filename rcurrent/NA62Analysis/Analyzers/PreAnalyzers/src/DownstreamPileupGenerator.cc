...
/// \Brief
/// Generates pileup from beam particles in the Spectrometer, NewCHOD, and MUV3.
/// \EndBrief
/// \Detailed
/// Pileup particles (beam and halo, separately) are taken from the PileupGenerator.
/// Beam particles (can) decay according to specified rules (e.g kaons decay in the 6 main modes).
/// For each pileup particle, an appropriate entry from one of the MC hits libraries is read.
/// The hits are then added into the MC event at the time determined in the PileupGenerator.
///
...
/// It is accessed via the NA62ConditionsService.
/// Each library contains O(10^5) events; each event contains 3 detectors; each detector contains
...
/// detectors (3) are hard-coded in the constructor of this class.
///
/// Histograms produced are:
///  "Gen_Time": the time of the pileup event, set by the PileupGenerator
///  "Gen_Entry": which event (number) was read from the library (should be flat)
///  "Gen_Kaon": number of kaon decays generated
///  "Gen_Pion": number of pion decays generated (currently zero)
///  "Gen_Halo": number of halo tracks generated (currently equal to number of kaon decays in 102-180m)
///  "Gen_Total": total number of decays and halo tracks generated
///  "nDetectorHits": 2D plot with number of Detector (Spectrometer, NewCHOD, MUV3) hits before and after
///    the pileup hits were added to the event
///  "KaonDecayMode": the fraction of kaon decays in each decay mode (mode 6 should be empty!)
///  "LibraryUsed": how often was each of the libraries used?
...
void DownstreamPileupGenerator::Process(int ){
  ...
  TRecoSpectrometerEvent* Spectrometerevent = GetEvent<TRecoSpectrometerEvent>("Reco");
  TRecoNewCHODEvent* NewCHODevent = GetEvent<TRecoNewCHODEvent>("Reco");
  TRecoMUV3Event* MUV3event = GetEvent<TRecoMUV3Event>("Reco");
  TRecoLKrEvent* LKrevent = GetEvent<TRecoLKrEvent>("Reco");
  TRecoLAVEvent* LAVevent = GetEvent<TRecoLAVEvent>("Reco");
  Int_t nSpectrometerHitsBeforePileup = Spectrometerevent->GetNHits();
  Int_t nNewCHODHitsBeforePileup = NewCHODevent->GetNHits();
  Int_t nMUV3HitsBeforePileup = MUV3event->GetNCandidates();
  Int_t nLKrHitsBeforePileup = LKrevent->GetNCandidates();
  Int_t nLAVHitsBeforePileup = LAVevent->GetNHits();
  ////////////////////////////////////////////////////////
  ...
  if(fGenerateHaloPileup){
    ...
    for( ; particle != haloParticles->end(); ++particle){
      Float_t time = particle->first;    
      GeneratePileup(time, DecodePID(3)); // Generate halo pileup!
    }
  }
  ...
  Int_t nSpectrometerHitsAfterPileup = Spectrometerevent->GetNHits();
  Int_t nNewCHODHitsAfterPileup = NewCHODevent->GetNHits();
  Int_t nMUV3HitsAfterPileup = MUV3event->GetNCandidates();
  Int_t nLKrHitsAfterPileup = LKrevent->GetNCandidates();
  Int_t nLAVHitsAfterPileup = LAVevent->GetNHits();
