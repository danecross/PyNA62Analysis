...
#include "SpectrometerRICHEventMonitor.hh"
...
/// \class SpectrometerRICHEventMonitor
/// \Brief
/// Visualization of the expected and reconstructed RICH rings
/// \EndBrief
/// \Detailed 
...
/// A series monitoring plots for the whole data sample are also produced.
/// \author Evgueni Goudzovski (eg@hep.ph.bham.ac.uk)
/// \EndDetailed

SpectrometerRICHEventMonitor::SpectrometerRICHEventMonitor(Core::BaseAnalysis *ba) :
  Analyzer(ba, "SpectrometerRICHEventMonitor") {

  RequestTree("RICH", new TRecoRICHEvent);
  RequestTree("Spectrometer", new TRecoSpectrometerEvent);

  fOutputPDFName = "./SpectrometerRICHEventMonitor.pdf";
  fMaxGraphCount = 100;
  fNTrackCutLow  = 1;
  fNTrackCutHigh = 3;
  fGraphCount    = 0;
  ...
  fH2 = new TH2F("h2", "h2", 1, -600, 300, 1, -450, 450);
  fGHits = new TGraph();
...
SpectrometerRICHEventMonitor::~SpectrometerRICHEventMonitor() {
  if (fH2)         delete fH2;
  if (fGHits)      delete fGHits;
  if (fCrown)      delete fCrown;
  if (fArc)        delete fArc;
...
void SpectrometerRICHEventMonitor::Process(Int_t iEvent) {
  ...
  for (Int_t i=0; i<nRichHits; i++) {
    ...
    if (hit->GetOrSuperCellID()==0) { // discard SuperCells
      nGoodRichHits++;
  ...
  std::vector<SpectrometerRICHAssociationOutputSimple> SpecRICH =
    *(std::vector<SpectrometerRICHAssociationOutputSimple>*)GetOutput("SpectrometerRICHAssociationSimple.Output");

  /////////////////////////////////////
  // Draw expected rings for each track

  for (UInt_t itrack=0; itrack<SpecRICH.size(); itrack++) {
    SpectrometerRICHAssociationOutputSimple sr = SpecRICH[itrack];
    ...
    if (sr.GetRingExpectedRadiusEl()>0.0) {
      fCrown->SetLineColor(kGreen+3);
      fCrown->SetFillColor(kGreen-7);
      fCrown->DrawCrown(xc, yc, sr.GetRingExpectedRadiusEl()-RadiusMargin, sr.GetRingExpectedRadiusEl()+RadiusMargin);
    }
    if (sr.GetRingExpectedRadiusMu()>0.0) {
      fCrown->SetLineColor(kGreen+2);
      fCrown->SetFillColor(kGreen-9);
      fCrown->DrawCrown(xc, yc, sr.GetRingExpectedRadiusMu()-RadiusMargin, sr.GetRingExpectedRadiusMu()+RadiusMargin);
    }
    if (sr.GetRingExpectedRadiusPi()>0.0) {
      fCrown->SetLineColor(kGreen);
      fCrown->SetFillColor(kGreen-10);
      fCrown->DrawCrown(xc, yc, sr.GetRingExpectedRadiusPi()-RadiusMargin, sr.GetRingExpectedRadiusPi()+RadiusMargin);
    }
    ...
    TString  IsElectron1 = sr.GetIsElectron1() ? "Y" : "N";
    TString  IsMuon1     = sr.GetIsMuon1()     ? "Y" : "N";
    TString  IsPion1     = sr.GetIsPion1()     ? "Y" : "N";
    TString  IsElectron2 = sr.GetIsElectron2() ? "Y" : "N";
    TString  IsMuon2     = sr.GetIsMuon2()     ? "Y" : "N";
    TString  IsPion2     = sr.GetIsPion2()     ? "Y" : "N";

    TString Report =
      Form("TR%d: P=%4.1f R_{e,#mu,#pi}=%3d/%3d/%3d N_{exp}=%4.1f/%4.1f/%4.1f N_{obs}=%2d/%2d/%2d ID=%s%s%s %s%s%s",
	   itrack, 1e-3*sr.GetTrackMomentum(),
	   (Int_t)sr.GetRingExpectedRadiusEl(), (Int_t)sr.GetRingExpectedRadiusMu(), (Int_t)sr.GetRingExpectedRadiusPi(),
    ...
    if (sr.GetIsElectron1()) FillHisto("hMomentum_IsElectron1", 1e-3*sr.GetTrackMomentum());
    else FillHisto("hMomentum_NotElectron1", 1e-3*sr.GetTrackMomentum());
    if (sr.GetIsMuon1()) FillHisto("hMomentum_IsMuon1", 1e-3*sr.GetTrackMomentum());
    else FillHisto("hMomentum_NotMuon1", 1e-3*sr.GetTrackMomentum());
    if (sr.GetIsPion1()) FillHisto("hMomentum_IsPion1", 1e-3*sr.GetTrackMomentum());
    else FillHisto("hMomentum_NotPion1", 1e-3*sr.GetTrackMomentum());

    if (sr.GetIsElectron2()) FillHisto("hMomentum_IsElectron2", 1e-3*sr.GetTrackMomentum());
    else FillHisto("hMomentum_NotElectron2", 1e-3*sr.GetTrackMomentum());
    if (sr.GetIsMuon2()) FillHisto("hMomentum_IsMuon2", 1e-3*sr.GetTrackMomentum());
    else FillHisto("hMomentum_NotMuon2", 1e-3*sr.GetTrackMomentum());
    if (sr.GetIsPion2()) FillHisto("hMomentum_IsPion2", 1e-3*sr.GetTrackMomentum());
  ...
  if (GetWithMC()) {
    ...
    for (Int_t ipart=0; ipart<Npart; ipart++) {
      Particle = static_cast<KinePart*>(KinePartsArray[ipart]);
...
void SpectrometerRICHEventMonitor::EndOfJobUser() {
