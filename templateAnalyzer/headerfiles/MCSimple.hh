...
public:
	...
	//Accessing elements
	std::vector<KinePart*> operator[](TString);
	std::vector<KinePart*> operator[](int);
	int Size(int pdgID) const;
	int Size(TString name) const;
	std::vector<KinePart*> GetFinalState();
	std::vector<KinePart*> GetDecayLevel(int level, bool full);
